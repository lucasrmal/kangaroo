...
Amount InvestmentTransaction::balanceAfterSplit(
    const Amount& _balanceBefore, const SplitFraction& _fraction) {
  return _fraction.first == _fraction.second
             ? _balanceBefore
...
void InvestmentTransaction::setSplits(
    const QList<KLib::Transaction::Split>& _splits) {
  // We only allow setSplits if all splits are exactly the same except for one,
  // to allow reassigning the transaction to a different account. Account may
  // only be the dividend src/dest, capital gain and has to be the same type.
  std::unordered_map<int, const Transaction::Split*> splits_hash;
  for (const Transaction::Split& split : _splits) {
    splits_hash[split.idAccount] = &split;
  }

  std::vector<Transaction::Split*> changed_existing_splits;
  for (Transaction::Split& split : m_splits) {
    if (splits_hash.contains(split.idAccount) &&
        splits_hash.at(split.idAccount)->equals(split)) {
      splits_hash.erase(split.idAccount);
    } else {
      changed_existing_splits.push_back(&split);
    }
  }

  if (splits_hash.empty()) {
    // No change, return.
    return;
  } else if (splits_hash.size() != 1 || changed_existing_splits.size() != 1) {
    ModelException::throwException(
        tr("Cannot call setSplits() on an InvestmentTransaction!"), this);
    return;
  }

  Transaction::Split* existing_split = *changed_existing_splits.begin();
  const Transaction::Split& modified_split = *splits_hash.begin()->second;

  if (existing_split->idAccount == modified_split.idAccount ||
      existing_split->currency != modified_split.currency ||
      existing_split->amount != modified_split.amount ||
      !Account::getAccount(modified_split.idAccount)
           ->supportsCurrency(modified_split.currency) ||
      Account::getAccount(existing_split->idAccount)->type() !=
          Account::getAccount(modified_split.idAccount)->type()) {
    ModelException::throwException(
        tr("Cannot call setSplits() on an InvestmentTransaction!"), this);
    return;
  }

  Transaction::Split old_split = *existing_split;
  existing_split->idAccount = modified_split.idAccount;
  emit splitRemoved(old_split);
  emit splitAdded(*existing_split);
...
const Transaction::Split& InvestmentTransaction::splitFor(
    InvestmentSplitType _type) const {
...
//    Amount InvestmentTransaction::balanceAfterSplit(const Amount&
//    balanceBefore) const
//    {
//        if (m_action == InvestmentAction::Split && _account ==
//        idInvestmentAccount())
//        {
//            return (balanceBefore * m_splitFraction.first) /
//            m_splitFraction.second;
//        }
//        else
//        {
//            return 0;
...
Amount InvestmentTransaction::netPricePerShare() const {
  if (!hasSplitFor(InvestmentSplitType::Fee)) return m_pricePerShare;
  ...
  switch (m_action) {
    ...
    case InvestmentAction::ReinvestDistrib:
      return m_pricePerShare +
             m_splits[m_types[InvestmentSplitType::Fee]].amount /
...
Amount InvestmentTransaction::fee() const {
  return hasSplitFor(InvestmentSplitType::Fee)
             ? m_splits[m_types[InvestmentSplitType::Fee]].amount
             : 0;
}

Amount InvestmentTransaction::gainLoss() const {
  return hasSplitFor(InvestmentSplitType::GainLoss)
             ? m_splits[m_types[InvestmentSplitType::GainLoss]].amount
             : 0;
}
...
void InvestmentTransaction::makeBuySellFee(
    InvestmentAction _action, const Amount& _pricePerShare,
    const QList<Split>& _splits, const QList<InvestmentSplitType>& _types,
    const Lots& _lots) {
  // Check action
  checkAction(_action, {InvestmentAction::Buy, InvestmentAction::Sell,
                        InvestmentAction::ShortSell,
                        InvestmentAction::ShortCover, InvestmentAction::Fee});

  QSet<InvestmentSplitType> optional = {InvestmentSplitType::Tax,
                                        InvestmentSplitType::Fee};

  if (_action == InvestmentAction::Sell ||
      _action == InvestmentAction::ShortCover) {
  ...
  // Check splits
  checkSplits(_splits, _types,
              /* req */
              {InvestmentSplitType::CostProceeds,
               InvestmentSplitType::Investment, InvestmentSplitType::Trading},
              optional);
  ...
  try {
    if (_action == InvestmentAction::Buy ||
        _action == InvestmentAction::ShortSell) {
      InvestmentLotsManager::instance()->updateTransactionSplit(this);
      m_lots.clear();
    } else if (_action != InvestmentAction::Fee) {
      InvestmentLotsManager::instance()->updateUsages(this, _lots);
      m_lots = _lots;
    }
  ...
  if (oldAction != _action) emit investmentActionChanged(oldAction);

  if (!onHoldToModify()) emit modified();
}

void InvestmentTransaction::makeTransferSwap(
    InvestmentAction _action, const QList<Split>& _splits,
    const QList<InvestmentSplitType>& _types, const Lots& _lots) {
  // Check action
  checkAction(_action, {InvestmentAction::Transfer, InvestmentAction::Swap});

  QSet<InvestmentSplitType> req = {InvestmentSplitType::InvestmentFrom,
...
void InvestmentTransaction::makeSpinoff(
    const QList<Split>& _splits, const QList<InvestmentSplitType>& _types,
    const Lots& _lots) {
  QSet<InvestmentSplitType> req = {InvestmentSplitType::Investment,
                                   InvestmentSplitType::InvestmentTo,
...
void InvestmentTransaction::makeSplit(int _idInvestmentAccount,
                                      const SplitFraction& _fraction) {
  // Check the investment account
  checkIdInvestmentAccount(_idInvestmentAccount);

  // Check the split fraction
  if (_fraction.first <= 0 || _fraction.second <= 0 ||
      _fraction.first == _fraction.second) {
...
void InvestmentTransaction::makeReinvestedDivDist(
    InvestmentAction _action, const Amount& _pricePerShare,
    const QList<Split>& _splits, const QList<InvestmentSplitType>& _types,
    const DistribComposition& _composition) {
  // Check action
  checkAction(_action, {InvestmentAction::ReinvestDiv,
  ...
  checkSplits(_splits, _types,
              ...
              {InvestmentSplitType::DistributionSource,
               InvestmentSplitType::Investment, InvestmentSplitType::Trading},
              /* opt */
              {InvestmentSplitType::Fee, InvestmentSplitType::Tax,
               InvestmentSplitType::CashInLieu});

  // Check composition if dist. (this will catch both sum errors and not empty
  // if Dividend errors)
...
void InvestmentTransaction::makeDivDist(
    InvestmentAction _action, int _idInvestmentAccount,
    const QList<Split>& _splits, const QList<InvestmentSplitType>& _types,
    const DistribComposition& _composition) {
  // Check action
  checkAction(_action,
  ...
  checkSplits(
      ...
      /* opt */ {InvestmentSplitType::Tax, InvestmentSplitType::Trading});
...
void InvestmentTransaction::makeCostBasisAdjustment(int _idInvestmentAccount,
...
void InvestmentTransaction::makeUndistributedCapitalGain(
    int _idInvestmentAccount, const Amount& _capitalGain,
...
void InvestmentTransaction::setDistribComposition(
    const DistribComposition& _composition) {
  if (m_action == InvestmentAction::Distribution ||
...
void InvestmentTransaction::checkDistribComposition(
    InvestmentAction _action, const DistribComposition& _composition) {
  ...
  if (_action != InvestmentAction::Distribution &&
    ...
    ModelException::throwException(tr("Distribution composition can only "
                                      "be set for Distribution or Reinvested "
                                      "Distribution transactions."),
                                   this);
  } else if (t != 100) {
    ModelException::throwException(
        tr("Distribution composition must add up to 100."), this);
  }
}

void InvestmentTransaction::checkSplits(
    const QList<Split>& _splits, const QList<InvestmentSplitType>& _types,
    const QSet<InvestmentSplitType>& _required,
    const QSet<InvestmentSplitType>& _optional, InvestmentAction _action) {
  // First, must balance
  ...
  if (_splits.count() != _types.count()) {
    ModelException::throwException(
        tr("There must be the same number of splits and types."), this);
  }
  ...
  for (int i = 0; i < _splits.count(); ++i) {
    if (_splits[i].amount == 0) {
      ModelException::throwException(tr("Cannot have zero amount splits!."),
                                     this);
    }
    ...
    switch (_types[i]) {
      case InvestmentSplitType::Investment:
        ...
        if (a->type() != AccountType::INVESTMENT) {
          ModelException::throwException(
              tr("Wrong account for this type (investment)."), this);
        }
        break;
      ...
      case InvestmentSplitType::InvestmentFrom:
        ...
        if (a->idSecurity() == Constants::NO_ID) {
          ModelException::throwException(
              tr("Wrong account for this type (investment from)."), this);
        }
        break;
      ...
      case InvestmentSplitType::InvestmentTo:
        ...
        if (a->idSecurity() == Constants::NO_ID) {
          ModelException::throwException(
              tr("Wrong account for this type (investment to)."), this);
        }
        break;
      ...
      case InvestmentSplitType::Trading:
        if (a->type() != AccountType::TRADING) {
          ModelException::throwException(
              tr("Wrong account for this type (trading)."), this);
        }
        break;
      ...
      default:
        if (a->type() == AccountType::INVESTMENT) {
          ModelException::throwException(
              tr("Wrong account for this type (non investment)."), this);
        }
        break;
    }
  }
  ...
  if (idAcctFrom == idAcctTo && idAcctFrom != Constants::NO_ID) {
    ModelException::throwException(
        tr("The source and destination accounts must differ."), this);
  }

  if (_action == InvestmentAction::Transfer && idSecurity != idSecurityOther) {
    ModelException::throwException(tr("The source and destination securities "
                                      "for the transfer must match."),
                                   this);
  } else if ((_action == InvestmentAction::Swap ||
              _action == InvestmentAction::Spinoff) &&
             idSecurity == idSecurityOther) {
    ModelException::throwException(
        tr("The source and destination securities for the swap must differ."),
        this);
  }

  // Next: must contain exactly one of each (except trading), contain all the
  // required, and be a subset of req \cup opt
  ...
  for (int i : counts) {
    if (i != 1) {
      ModelException::throwException(
          tr("At most one split of each type (except Trading) is allowed."),
          this);
    }
  }
  ...
  if (!types.contains(_required)) {
    ModelException::throwException(
        tr("At least one required split is missing."), this);
  }

  if (!(_required | _optional).contains(types))  // Operator Union. It's ok ;-)
  {
    ModelException::throwException(
        tr("The split types must be a subset of the required "
           "and optional types for this investment transaction action."),
        this);
  }
...
void InvestmentTransaction::checkAction(
    InvestmentAction _action, const QSet<InvestmentAction>& _allowed) {
  if (!_allowed.contains(_action)) {
    ModelException::throwException(tr("Invalid investment transaction action!"),
                                   this);
  }
}
...
void InvestmentTransaction::checkIdInvestmentAccount(int _idInvestmentAccount) {
  ...
  if (a->type() != AccountType::INVESTMENT) {
    ModelException::throwException(
        tr("The investment split must relate to an investment account."), this);
  }
}
...
void InvestmentTransaction::emitSplitSignals(const QList<Split>& _old,
                                             const QList<Split>& _new) {
...
QString InvestmentTransaction::autoMemo() const {
  ...
  switch (m_action) {
    case InvestmentAction::Buy:
      memo = tr("Bought %1 shares at %2 per share. Total cost: %3")
                 .arg(shareCount().toString())
                 .arg(pricePerShare().toString())
                 .arg(splitFor(InvestmentSplitType::CostProceeds)
                          .invertedFormattedAmount());
      break;
    case InvestmentAction::Sell:
      memo = tr("Sold %1 shares at %2 per share. Total proceeds: %3")
                 .arg((-1 * shareCount()).toString())
                 .arg(pricePerShare().toString())
                 .arg(splitFor(InvestmentSplitType::CostProceeds)
                          .formattedAmount());
      break;
    ...
    case InvestmentAction::Distribution:
      memo = tr("%1: %2")
                 ...
                 .arg(splitFor(InvestmentSplitType::DistributionSource)
                          .invertedFormattedAmount());
      break;
    case InvestmentAction::ReinvestDiv:
    case InvestmentAction::ReinvestDistrib:
      memo = tr("Reinvested %1: %2 shares at %3 per share.")
                 .arg(splitFor(InvestmentSplitType::DistributionSource)
                          .invertedFormattedAmount())
                 .arg(shareCount().toString())
                 .arg(pricePerShare().toString());
    ...
    case InvestmentAction::Fee:
      memo = tr("Fee: %1")
                 .arg(actionToString(m_action))
                 .arg(splitFor(InvestmentSplitType::Fee).formattedAmount());
      break;
    default:
      break;
  }
  ...
  if (!memo.isEmpty()) {
    memo = QString("[%1] %2")
               .arg(SecurityManager::instance()
                        ->get(Account::getTopLevel()
...
void InvestmentTransaction::load(QXmlStreamReader& _reader) {
  ...
  switch (m_action) {
    ...
    case InvestmentAction::Fee:
      m_pricePerShare =
          Amount::fromStoreable(IO::getAttribute("pricepershare", attributes));
      break;

    case InvestmentAction::StockSplit: {
      QStringList frac = IO::getAttribute("splitfraction", attributes)
    ...
    {
      QStringList l = IO::getAttribute("distribcomposition", attributes)
                          .split(",", QString::SkipEmptyParts);
      m_distribComposition.clear();
      ...
      for (const QString& s : l) {
        ...
        if (d.size() != 2) {
          throw IOException(
              tr("Invalid value for the distribution composition."));
        }
        m_distribComposition.insert((DistribType)d[0].toInt(),
                                    Amount::fromStoreable(d[1]));
      }
      break;
    }
    case InvestmentAction::UndistributedCapitalGain:
      m_taxPaid = Amount::fromStoreable(
          IO::getAttribute("taxpaid", attributes));  // Pass through!

    case InvestmentAction::CostBasisAdjustment:  // Yes fall through
      m_basisAdjustment = Amount::fromStoreable(
          IO::getAttribute("basisadjustment", attributes));
      break;
  ...
  if (_reader.readNextStartElement() &&
    ...
    throw IOException(
        tr("Transaction element missing for an investment transaction!."));
  }
...
void InvestmentTransaction::save(QXmlStreamWriter& _writer) const {
  ...
  switch (m_action) {
    ...
    case InvestmentAction::StockSplit:
      _writer.writeAttribute("splitfraction", QString("%1:%2")
                                                  .arg(m_splitFraction.first)
    ...
    case InvestmentAction::ReinvestDistrib:
      _writer.writeAttribute(
          "pricepershare",
          m_pricePerShare.toStoreable());  // Yes fall through
    case InvestmentAction::Distribution: {
      QStringList l;

      for (auto i = m_distribComposition.begin();
           i != m_distribComposition.end(); ++i) {
    ...
    case InvestmentAction::UndistributedCapitalGain:
      _writer.writeAttribute("taxpaid",
                             m_taxPaid.toStoreable());  // Yes fall through
    case InvestmentAction::CostBasisAdjustment:
      _writer.writeAttribute("basisadjustment",
                             m_basisAdjustment.toStoreable());
      break;
