...
void TransactionManager::remove(int _id) {
  if (m_transactions.contains(_id)) {
    ...
    if (qobject_cast<InvestmentTransaction*>(trans)) {
      InvestmentLotsManager::instance()->removeTransaction(
          qobject_cast<InvestmentTransaction*>(trans));
    }
...
void TransactionManager::reassignTransactions(
    const std::vector<int>& transaction_ids, int reassign_from, int reassign_to,
    QStringList* errors) {
  for (int transaction_id : transaction_ids) {
    ...
    for (Transaction::Split& split : splits) {
      if (split.idAccount == reassign_from) {
        split.idAccount = reassign_to;
        modified = true;
      }
    }
    ...
    if (modified) {
      try {
        ...
        errors->push_back(tr("Unable to update transaction %1: %2")
                              .arg(transaction_id)
                              .arg(e.description()));
      }
    }
...
int TransactionManager::newId() { return m_nextId++; }
...
void TransactionManager::load(QXmlStreamReader& _reader) {
  ...
  //        //Otherwise, create an investment transaction from the existing
  //        transaction. Account* investmentAccount = nullptr;
  ...
  //                TransactionType::Invest_Transfer)
  //                {
  ...
  //                     && transaction->type() ==
  //                     TransactionType::Invest_Reinvest
  ...
  //        default:
  //            throw IOException(QObject::tr("Stock Splits Conversion is Not
  //            Supported!"));
  //        }
  ...
  while (!(_reader.tokenType() == QXmlStreamReader::EndElement &&
           _reader.name() == StdTags::TRANSACTION_MGR)) {
    if (_reader.tokenType() == QXmlStreamReader::StartElement &&
      ...
      //            catch (ModelException e)
      //            {
      //                throw IOException(tr("Exception occured while
      //                transforming transaction %1 "
      //                                     "to an investment
      ...
      connect(o, &Transaction::modified, this, &TransactionManager::modified);
               ...
               _reader.name() == StdTags::INVEST_TRANSACTION) {
...
void TransactionManager::afterLoad() {
  for (Transaction* t : m_transactions) {
    ...
    //            {
    //                s.currency =
    //                Account::getTopLevel()->account(s.idAccount)->mainCurrency();
    //            }
    //        }
  }
}
