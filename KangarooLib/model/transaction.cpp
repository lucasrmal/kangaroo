...
namespace KLib
{
    ...
    void Transaction::setAttachments(const QSet<int>& _attachments)
    {
        ...
        for (int docId : _attachments)
        {
            ...
            {
                ModelException::throwException(tr("Document %1 does not exists.").arg(docId), this);
                return;
            }
        }
        ...
        for (int docId : m_attachments)
        {
            ...
            catch (...) {} //If exception, then document does not exist anyways, which is OK.
        }
    ...
    Amount Transaction::totalForAccount(int _idAccount, const QList<Split>& _splits)
    {
        Amount tot = 0;
    ...
    Balances Transaction::totalsForAccount(int _idAccount, const QList<Split>& _splits)
    {
        Balances tot;
    ...
    QString Transaction::splitsImbalances(const QList<KLib::Transaction::Split>& _splits)
    {
        QHash<QString, Amount> totals;
        ...
        for (const Split& s : _splits)
        {
            ...
            else
            {
                ...
                {
                    totals[SecurityManager::instance()->get(a->idSecurity())->symbol()] += s.amount;
                }
            }
        }
    ...
    bool Transaction::isCurrencyExchange(const QList<KLib::Transaction::Split>& _splits)
    {
        if (_splits.count() == 4)
        {
            ...
            for (const Split& s : _splits)
            {
                ...
                if (Account::getTopLevel()->account(s.idAccount)->type() == AccountType::TRADING)
                {
                    ++numTrading;
                }
            }
    ...
    {
        return totalFor(_idAccount).inCurrency(Account::getTopLevel()->mainCurrency());
    }

    Amount Transaction::totalForInAccountCurrency(int _idAccount) const
    {
        return totalFor(_idAccount).inCurrency(Account::getTopLevel()->account(_idAccount)->mainCurrency());
    }
    ...
    void Transaction::checkIfCurrencyExchange()
    {
        ...
        m_isCurrencyExchange = currencies.count() == 2 && numTrading == 2 && m_splits.count() == 4;
    }
    ...
    void Transaction::load(QXmlStreamReader& _reader)
    {
        ...
        m_clearedStatus = IO::getOptAttribute("cleared", attributes, ClearedStatus::None).toInt();
        ...
        m_idPayee       = IO::getOptAttribute("payee", attributes, Constants::NO_ID).toInt();

        QStringList attachments = IO::getOptAttribute("attachments", attributes, QString()).split(",", QString::SkipEmptyParts);
        ...
        {
            while (!(_reader.tokenType() == QXmlStreamReader::EndElement && _reader.name() == StdTags::TRANSACTION))
            {
                if (_reader.tokenType() == QXmlStreamReader::StartElement && _reader.name() == StdTags::SPLIT)
                {
                    Split s;
                    attributes = _reader.attributes();

                    s.amount    = Amount::fromStoreable(IO::getAttribute("amount", attributes));
                ...
                else if (_reader.tokenType() == QXmlStreamReader::StartElement && _reader.name() == StdTags::PROPERTIES)
                {
