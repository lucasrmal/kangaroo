...
enum Status { None = 0, Cleared, Reconciled, NumStatus };
...
class Transaction : public IStored {
 ...
 public:
  class Split {
   public:
    Split(const KLib::Amount& _amount, int _idAccount, const QString& _currency,
   ...
   * @see clearedStatus()
   * @param _status The status. Must be valid, otherwise will throw a
   * ModelException.
   */
   ...
   * @param _idPayee The payee id (must exist or will throw ModelException), or
   * Constants::NO_ID if no payee
   */
   ...
   * The list of attachments passed must be valid. An exception will be throwed
  ...
  Q_INVOKABLE virtual const QList<KLib::Transaction::Split>& splits() const {
    return m_splits;
  }
  Q_INVOKABLE virtual KLib::Transaction::Split split(int _i) const;

  Q_INVOKABLE virtual void setSplits(
  ...
  Q_INVOKABLE virtual KLib::Amount totalForInAccountCurrency(
      int _account) const;
  ...
  /**
    ...
    A transaction is a currency exchange if it is a transfer from one account to
    another, with the accounts using distinct currencies, such that the currency
    transfer goes through 2 trading accounts. Thus, the transaction has exactly
    4 splits, 2 of them being with trading accounts.
  */
  Q_INVOKABLE virtual bool isCurrencyExchange() const {
    return m_isCurrencyExchange;
  }

  /**
   * @brief A banner transaction is a transaction with a single split and 0
   * amount. Useful for "closing" credit card statements.
   */
  virtual bool isBannerTransaction() const { return m_splits.count() == 1; }

  /**
   * @return A custom color to use for the transaction, or empty string to use
   * the defaults.
   */
   ...
   */
  static Amount totalForAccount(int _idAccount,
                                const QList<KLib::Transaction::Split>& _splits);
  static Balances totalsForAccount(
      int _idAccount, const QList<KLib::Transaction::Split>& _splits);
  static QString splitsImbalances(
      const QList<KLib::Transaction::Split>& _splits);
  static bool isCurrencyExchange(
      const QList<KLib::Transaction::Split>& _splits);
