...
SplitsController::SplitsController(QList<Transaction::Split>& _splits,
                                   bool _readOnly, QObject* _parent)
    : QAbstractTableModel(_parent), m_splits(_splits), m_isReadOnly(_readOnly) {
  ensureOneEmptyRow();
}
...
QVariant SplitsController::data(const QModelIndex& _index, int _role) const {
  if (_role != Qt::EditRole && _role != Qt::DisplayRole) return QVariant();
  ...
  if (_index.isValid() && _index.row() < rowCount() && _role == Qt::EditRole) {
    switch (_index.column()) {
      ...
      case SplitsColumn::TRANSFER:
        ...
        try {
          m_splits[_index.row()].currency =
              Account::getTopLevel()->account(_value.toInt())->mainCurrency();
        } catch (...) {
        }
      ...
      case SplitsColumn::DEBIT: {
        ...
        if (a == 0 && m_splits[_index.row()].amount <
...
bool SplitsController::rowIsEmpty(int _row) {
  if (_row < 0 || _row >= rowCount()) return false;

  return m_splits[_row].amount == 0 && m_splits[_row].memo.isEmpty() &&
...
bool SplitsController::insertRows(int _row, int _count,
...
bool SplitsController::removeRows(int _row, int _count,
                                  const QModelIndex& _parent) {
...
void SplitsWidgetDelegate::setEditorData(QWidget* _editor,
                                         const QModelIndex& _index) const {
  switch (_index.column()) {
    case SplitsColumn::DEBIT:
    case SplitsColumn::CREDIT: {
...
void SplitsWidgetDelegate::updateEditorGeometry(
...
SplitsWidget::SplitsWidget(QList<Transaction::Split>& _splits, bool _readOnly,
                           QWidget* parent)
    : QTableView(parent),
  ...
  horizontalHeader()->setSectionResizeMode(SplitsColumn::MEMO,
                                           QHeaderView::Stretch);
...
void SplitsWidget::onDataChanged(const QModelIndex& _from,
                                 const QModelIndex& _to) {
  if (_from.column() <= (int)SplitsColumn::CREDIT &&
      _to.column() >= (int)SplitsColumn::DEBIT) {
    emit amountChanged();
  }
}
...
bool SplitsWidget::validate() {
    ...
    if (s.idAccount != Constants::NO_ID || s.amount != 0 || !s.memo.isEmpty()) {
      if (s.idAccount == Constants::NO_ID) {
        QMessageBox::information(
            this, msgBoxTitle, tr("Please select an account for this split."));
        edit(m_model->index(currentIndex().row(), SplitsColumn::TRANSFER));
        return false;
      } else if (s.amount == 0) {
        QMessageBox::information(this, msgBoxTitle,
                                 tr("Please enter an amount for this split"));
  ...
  try {
    if (notEmpty.count() < 2) {
      QMessageBox::information(this, msgBoxTitle,
                               tr("There must be at least two splits."));
      return false;
    }
    ...
    if (!Transaction::splitsBalance(notEmpty)) {
      QMessageBox::information(
          this, msgBoxTitle,
          tr("The splits do not balance. Debits must equal credits."));
      return false;
    }
...
void SplitsWidget::addRow() { m_model->insertRow(m_model->rowCount()); }

void SplitsWidget::removeCurrentRow() {
  m_model->removeRow(currentIndex().row());
}

void SplitsWidget::reload() { m_model->reload(); }
