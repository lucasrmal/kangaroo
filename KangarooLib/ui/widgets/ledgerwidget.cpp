...
LedgerAction::LedgerAction(QAction* _action, bool _isPrimary, sel_func _selChanged, trig_func _trig) :
  action(_action),
...
LedgerWidgetHorizontalHeader::LedgerWidgetHorizontalHeader(LedgerController* _controller, QWidget* _parent)
  : QHeaderView(Qt::Horizontal, _parent),
    m_controller(_controller)
{
}

void LedgerWidgetHorizontalHeader::paintSection(QPainter* _painter, const QRect& _rect, int _logicalIndex) const
{
  ...
  if (_logicalIndex == m_controller->col_flag())
  {
    ...
    //            _painter->drawPixmap(drawingRect, Core::pixmap("flag-red-16"));
...
LedgerWidget::LedgerWidget(Account *_account, QWidget *parent) :
  ...
  m_controller(LedgerControllerManager::instance()->ledgerFor(m_account, this))
//m_overlay(nullptr)
{
  if (!m_controller)
  {
  ...
  //setStyleSheet("QTreeView {alternate-background-color: #E1F0F3;background-color: #F5FBFF; }");
  setStyleSheet("QTreeView {alternate-background-color: #F1F8FF;background-color: #F6FCFF; } ");
  ...
  connect(m_delegate, &LedgerWidgetDelegate::editStarted,   this, &LedgerWidget::onEditStarted);

  connect(m_controller, &LedgerController::changesDiscarded, this, &LedgerWidget::checkActions);
  connect(m_controller, &LedgerController::changesSaved,     this, &LedgerWidget::checkActions);
  connect(m_controller, &LedgerController::showMessage, this, &LedgerWidget::showMessage);
...
LedgerWidget::~LedgerWidget()
{
  saveColumnWidths();
}

void LedgerWidget::rowsInserted(const QModelIndex& _parent, int _start, int _end)
{
                           ...
                           const QModelIndex& _index) const
{
  ...
  if (_index.isValid())
  {
    ...
    {
      newOption.features = newOption.features | (QStyleOptionViewItem::Alternate);
    }
    else //Even
    {
      newOption.features = newOption.features & (~QStyleOptionViewItem::Alternate);
...
bool LedgerWidget::handleTabEvent()
{
  ...
  while (!(m_controller->flags(m_controller->index(/* row    */ cur_idx.row(),
                                                   /* column */ nextCol,
                                                   /* parent */ cur_idx.parent())) & Qt::ItemIsEditable)
  ...
  else
  {
    ...
    if (wasEdit && !cur_idx.parent().isValid() && m_controller->rowCount(cur_idx)) //Parent to child
    {
      setCurrentIndex(m_controller->firstEditableIndex(m_controller->index(/* row    */ 0,
                                                                           /* column */ 0,
                                                                           /* parent */ cur_idx)));
    }
    else if (wasEdit && cur_idx.parent().isValid() && m_controller->rowCount(cur_idx.parent()) > cur_idx.row()+1)  //Child to child
    {
      setCurrentIndex(m_controller->firstEditableIndex(m_controller->index(/* row    */ cur_idx.row()+1,
                                                                           /* column */ 0,
                                                                           /* parent */ cur_idx.parent())));
    }
    else if (!wasEdit       //Otherwise, if there is no modified row, or if there is and we submit succesfully...
             || tryToSubmit())
    {
      const int mainRow = cur_idx.parent().isValid() ? cur_idx.parent().row()
                                                     : cur_idx.row();

      //Note that tryToSubmit will add a row if it's the last, so we're good :-)
      if (mainRow < m_controller->rowCount() -1)
      {
        setCurrentIndex(m_controller->firstEditableIndex(m_controller->index(/* row    */ mainRow+1,
                                                                             /* column */ 0)));
      }
      else if (wasEdit)
      {
        setCurrentIndex(m_controller->firstEditableIndex(m_controller->index(/* row    */ mainRow,
                                                                             /* column */ 0)));
      }
      else
      {
...
bool LedgerWidget::handleBacktabEvent()
{
  ...
  else
  {
    ...
    if (wasEdit && cur_idx.parent().isValid() && cur_idx.row() == 0) //Child to parent
    {
      setCurrentIndex(m_controller->lastEditableIndex(m_controller->index(/* row    */ cur_idx.parent().row(),
                                                                          /* column */ 0)));
    }
    else if (wasEdit && cur_idx.parent().isValid() && cur_idx.row() > 0)  //Child to child
    {
      setCurrentIndex(m_controller->lastEditableIndex(m_controller->index(/* row    */ cur_idx.row()-1,
                                                                          /* column */ 0,
                                                                          /* parent */ cur_idx.parent())));
    }
    else if (!wasEdit       //Otherwise, if there is no modified row, or if there is and we submit succesfully...
             || tryToSubmit())
    {
      ...
      if (mainRow > 0)
      {
        setCurrentIndex(m_controller->lastEditableIndex(m_controller->index(/* row    */ mainRow-1,
                                                                            /* column */ 0)));
      }
      else
      {
...
void LedgerWidget::closeEditor(QWidget* _editor, QAbstractItemDelegate::EndEditHint _hint)
{
...
void LedgerWidget::keyPressEvent(QKeyEvent* _event)
{
  ...
  case Qt::Key_Return:
    if (m_controller->hasModifiedRow())
    {
      ...
      const int mainRow = currentIndex().parent().isValid() ? currentIndex().parent().row()
                                                            : currentIndex().row();
      ...
      {
        _event->accept();
        setCurrentIndex(m_controller->index(/* row    */ mainRow,
  ...
  case Qt::Key_Backtab:
    ...
    //        default:
    //            if (currentIndex().isValid() && edit(currentIndex(), EditKeyPressed, _event))
    //            {
    //                _event->accept();
    //                return;
    //            }
...
void LedgerWidget::mousePressEvent(QMouseEvent* _event)
{
  ...
  const int           mainRow = index.parent().isValid() ? index.parent().row()
                                                         : index.row();

  //First, check if we are trying to to to a row other than the current row,
  //try to save the transaction if it is the case.
  if (m_controller->hasModifiedRow() && (!index.isValid() || mainRow != m_controller->modifiedRow()))
  {
    if (m_delegate->currentEditor())
    {
      commitData(m_delegate->currentEditor());
  ...
  //Look if we are clicking on a flag/cleared status column, in which case the row should NOT be selected.
  if (_event->button() == Qt::LeftButton && m_controller->doClickEdition(index))
  {
    _event->accept();
    return;
  }
...
{
  if (_event && _event->button() == Qt::RightButton && indexAt(_event->pos()).isValid())
  {
    //Open popup menu
...
bool LedgerWidget::edit(const QModelIndex& _index, EditTrigger _trigger, QEvent* _event)
{
  const int mainRow = _index.parent().isValid() ? _index.parent().row()
                                                : _index.row();

  if (m_controller->hasModifiedRow() && mainRow != m_controller->modifiedRow())
  {
    //Do not allow editing on cells that do not belong to the currently edited row
    return false;
  }
...
void LedgerWidget::selectionChanged(const QItemSelection& _selected, const QItemSelection& _deselected)
{
  ...
  {
    selRows.insert(i.parent().isValid() ? i.parent().row()
                                        : i.row());
  }

  for (const QModelIndex& i : _deselected.indexes())
  {
    deselectRows.insert(i.parent().isValid() ? i.parent().row()
                                             : i.row());
  }

  if (m_controller->modifiedRow() != -1 && !selRows.contains(m_controller->modifiedRow()))
  {
  ...
  //            m_overlay->move(width() - 301, indexRect.bottom() + horizontalHeader()->height() + 3);
...
bool LedgerWidget::tryToSubmit()
{
  ...
  if (!m_controller->submitChanges(wrongColumn))
  {
    ...
    QModelIndex idx = m_controller->index(/* row    */ m_controller->modifiedRow(),
                                          /* column */ wrongColumn);
...
void LedgerWidget::createBasicActions()
{
  ...
  auto askForDelete = [this]()
  {
    ...
    scheduleOptions << (numSchedules > 1 ? tr("Skip these occurrences") : tr("Skip this occurrence"));
    scheduleOptions << (numSchedules > 1 ? tr("Skip these occurrences and cancel all future occurences of the schedules")
                                         : tr("Skip this occurrence and cancel all future occurences of the schedule"));
    ...
    {
      QString title = numSchedules > 0 ? tr("Delete Schedules")
                                       : tr("Delete Schedule");

      QString msg = tr("What would you like to do?");

      if (numSchedules > 1) msg += " " + tr("This action will be applied to all %1 selected schedules.").arg(numSchedules);

      int answer = OptionsDialog::askForOptions(Core::pixmap("trash"),
                                                title,
                                                msg,
                                                scheduleOptions,
                                                0,
    ...
    else if (numTransactions > 0 && numSchedules > 0)
    {
      ...
      QString msg = tr("Would you like to delete the selected transaction(s), and apply one of the following options"
  ...
  m_basicActions[BasicLedgerAction::NewTransaction] = new LedgerAction(new QAction(Core::icon("new-row"), tr("&New Transaction"), this), true,
                                                                       [this](const QList<int>&)
  {
  ...
  m_basicActions[BasicLedgerAction::NewBillReminder] = new LedgerAction(new QAction(Core::icon("new-bill"), tr("New &Bill Reminder"), this), true,
                                                                        [this](const QList<int>&)
  {
    return m_account->isOpen();
  },
  ...
  m_basicActions[BasicLedgerAction::NewSchedule] = new LedgerAction(new QAction(Core::icon("new-schedule"), tr("Ne&w Schedule"), this), true,
                                                                    [this](const QList<int>&)
  {
    return m_account->isOpen();
  },
  ...
  m_basicActions[BasicLedgerAction::Edit] = new LedgerAction(new QAction(Core::icon("edit"), tr("&Edit"), this), true,
  ...
  m_basicActions[BasicLedgerAction::Delete] = new LedgerAction(new QAction(Core::icon("trash"), tr("&Delete"), this), true,
                                                               [this](const QList<int>& rows)
  {
    return m_account->isOpen()
        && (rows.count() > 1
  ...
  m_basicActions[BasicLedgerAction::Duplicate] = new LedgerAction(new QAction(Core::icon("copy"), tr("D&uplicate"), this), true,
                                                                  [this](const QList<int>& rows)
  {
    return m_account->isOpen()
        && rows.count() == 1
  ...
  LedgerAction* actEnter = new LedgerAction(new QAction(Core::icon("enter-modifications"), tr("En&ter"), this), true,
                                            [this](const QList<int>& rows)
  {
    return m_account->isOpen()
        && rows.count() == 1
  ...
  LedgerAction* actDiscard = new LedgerAction(new QAction(Core::icon("erase"), tr("&Cancel Edit"), this), true,
                                              [this](const QList<int>& rows)
  {
    return m_account->isOpen()
        && rows.count() == 1
  ...
  LedgerAction* actSchedule = new LedgerAction(new QAction(Core::icon("schedule"), tr("Sc&hedule"), this), true,
                                               [this](const QList<int>& rows)
  {
    return m_account->isOpen()
        && rows.count() == 1
        && !rowIsLocked(rows.first())
        && ((m_controller->hasModifiedRow() && m_controller->rowIsSchedule(rows.first()))
            || (!m_controller->hasModifiedRow() && rows.first() != newTransactionRow()));
  },
  [this]() {
  ...
  LedgerAction* actAttachments = new LedgerAction(new QAction(Core::icon("attachment"), tr("&Attachments"), this), false,
                                                  [this](const QList<int>& rows)
  {
    return m_account->isOpen()
        && rows.count() == 1
  ...
  [this]() {
    m_controller->editAttachments(currentIndex());
  }
  );

  LedgerAction* actNote = new LedgerAction(new QAction(Core::icon("note"), tr("&Note"), this), false,
                                           [this](const QList<int>& rows)
  {
    return m_account->isOpen()
        && rows.count() == 1
  ...
  [this]() {
    m_controller->editNote(currentIndex());
  }
  );
...
void LedgerWidget::saveColumnWidths()
{
  ...
  //        settings.setValue("Geometry/LedgerWidget/ColumnWidths", QVariant::fromValue<QList<int> >(widths));
}

bool LedgerWidget::inEdit() const
{
...
{
  return m_controller->hasModifiedRow() && currentIndex().row() == m_controller->newTransactionRow();
}
...
QList<int> LedgerWidget::selectedRows() const
{
  ...
  for (const QModelIndex& i : selectedIndexes())
  {
    rows.insert(i.parent().isValid() ? i.parent().row()
                                     : i.row());
  }
...
void LedgerWidget::editCurrent()
{
  ...
  {
    QModelIndex idx = m_controller->index(/* row */ currentIndex().parent().isValid()
...
{
  setCurrentIndex(m_controller->index(m_controller->newTransactionRow(), m_controller->col_date()));
  QTreeView::edit(m_controller->index(m_controller->newTransactionRow(), m_controller->col_date()));
}
...
void LedgerWidget::checkActions()
{
  checkEnableActions(selectedRows());
}
...
void LedgerWidget::deleteSelected(DeleteScheduleAction _action)
{
  ...
  for (int id : transIds)
  {
    ...
    {
      errors << tr("Unable to delete transaction %1: %2").arg(id).arg(e.description());
    }
  }
  ...
  case DeleteScheduleAction::SkipCurrentAndFutureInstances:
    for (auto i = schedules.begin(); i != schedules.end(); ++i)
    {
      ...
      {
        errors << tr("Unable to delete future occurrences of the schedule %1: %2")
                  .arg(i.key())
                  .arg(e.description());
      }
    }
  ...
  case DeleteScheduleAction::SkipInstance:
    for (const QPair<int, QDate>& pair : schIds)
    {
      ...
      {
        errors << tr("Unable to delete occurrences of the schedule %1 on %2: %3")
                  .arg(pair.first)
...
{
  if (selectedRows().count() == 1
      && m_controller->duplicateTransaction(currentIndex(), m_delegate->lastUsedDate()))
  {
    QTimer::singleShot(1, this, SLOT(scrollToBottom()));
    QTreeView::edit(m_controller->index(m_controller->newTransactionRow(), m_controller->col_date()));
  }
