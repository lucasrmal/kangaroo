...
MultiInvestmentModel::MultiInvestmentModel(const QLinkedList<Account*>& _accounts, QObject* _parent)
...
QVariant MultiInvestmentModel::data(const QModelIndex& _index, int _role) const
{
  if ((_role != Qt::DisplayRole && _role != Qt::EditRole) || !_index.isValid() || _index.row() >= rowCount())
  {
    return QVariant();
  }
...
QVariant MultiInvestmentModel::headerData(int _section, Qt::Orientation _orientation, int _role) const
{
...
{
  if (_index.isValid() && _index.row() <= rowCount() && _index.column() != COLUMN_ACCOUNT)
  {
...
bool MultiInvestmentModel::setData(const QModelIndex& _index, const QVariant& _value, int _role)
{
  if (_role != Qt::EditRole || !_index.isValid() || _index.row() >= rowCount())
  {
    return false;
  }
  ...
  case COLUMN_SHARES:
    info.shares = Amount::fromUserLocale(_value.toString(), info.security()->precision());
    break;

  case COLUMN_AMOUNT:
    info.amount = Amount::fromUserLocale(_value.toString(), info.currency()->precision());
    info.pricePerShare = 0;
    break;

  case COLUMN_PRICE_PER_SHARE:
    info.pricePerShare = Amount::fromUserLocale(_value.toString(), info.security()->precision());
    info.amount = 0;
    break;
  ...
  emit dataChanged(index(_index.row(), 0), index(_index.row(), columnCount()-1));
  return true;
}

void MultiInvestmentModel::save(const QDate& _date, const Account* m_brokerageAccount)
{
  for (RowInfo& info : m_rows)
  {
    if (info.shares != 0 && (info.amount != 0 || info.pricePerShare != 0))
    {
      if (info.pricePerShare == 0)
      {
        info.pricePerShare = info.amount.toPrecision(info.security()->precision()) / info.shares.toDouble();
      }
      else
      {
        info.amount = (info.pricePerShare * info.shares).toPrecision(info.currency()->precision());
      }

      QList<Transaction::Split> splits;
      QList<InvestmentSplitType> types;

      splits << Transaction::Split(-info.amount, m_brokerageAccount->id(), info.currency()->code());
      ...
      transaction->makeBuySellFee(InvestmentAction::Buy, info.pricePerShare, splits, types);
      LedgerManager::instance()->addTransaction(transaction);
    }
  }
}

FormMultiInvestmentEntry::FormMultiInvestmentEntry(int _brokerageAccountId, QWidget* _parent) :
...
void FormMultiInvestmentEntry::accept()
{
  ...
  for (const RowInfo& info : m_model->rowInfo())
  {
    if (info.shares != 0)
    {
      ...
      {
        errors << tr("Enter an amount or a price per share for %1.").arg(info.account()->name());
      }
    }
    else if (info.amount != 0 || info.pricePerShare != 0)
    {
      errors << tr("Enter a number of shares for %1.").arg(info.account()->name());
    }
  }
  ...
  catch (ModelException e)
  {
    QMessageBox::warning(this, tr("Save Changes"),
                         tr("An error occured while saving the transaction(s): %1").arg(e.description()),
                         QMessageBox::Ok);
  }
}
...
void FormMultiInvestmentEntry::updateTotal()
{
  ...
  for (const RowInfo& info : m_model->rowInfo())
  {
    if (info.shares != 0)
    {
      Amount amt = info.amount != 0 ? info.amount : info.shares * info.pricePerShare;
      total.add(info.currency()->code(), amt.toPrecision(info.currency()->precision()));
    }
  }
...
void FormMultiInvestmentEntry::createModel()
{
  ...
  // If the parent of this account is a placeholder and has only 1 brokerage account, use the parent. Otherwise, use
  // this account.
  bool useParent = false;
  if (m_brokerageAccount->hasParent() && m_brokerageAccount->parent()->isPlaceholder())
  {
    int numBrokerage = 0;
  ...
  findAccounts = [this, &accounts, &findAccounts] (Account* account) {
    if (account->type() == AccountType::INVESTMENT &&
        account->isOpen() && !account->isPlaceholder() &&
        m_brokerageAccount->allCurrencies().contains(
  ...
  m_transactionTable->setItemDelegate(new MultiInvestmentEntryDelegate(m_model, this));
  ...
  m_transactionTable->setColumnWidth(MultiInvestmentModel::COLUMN_PRICE_PER_SHARE, 100);
  m_transactionTable->horizontalHeader()->setSectionResizeMode(MultiInvestmentModel::COLUMN_MEMO, QHeaderView::Stretch);

  connect(m_model, SIGNAL(dataChanged(QModelIndex,QModelIndex)), this, SLOT(updateTotal()));
}

MultiInvestmentEntryDelegate::MultiInvestmentEntryDelegate(const MultiInvestmentModel *_model, QObject *_parent) :
...
QSize MultiInvestmentEntryDelegate::sizeHint(const QStyleOptionViewItem& _option, const QModelIndex& _index) const
{
    return QSize(QStyledItemDelegate::sizeHint(_option, _index).width(), m_rowHeight);
}

QWidget* MultiInvestmentEntryDelegate::createEditor(QWidget* _parent,
                                                    ...
                                                    const QModelIndex& _index) const
{
  ...
  {
    AmountEdit* amtEdit = new AmountEdit(m_model->rowInfo().at(_index.row()).security()->precision(), _parent);
  ...
  {
    AmountEdit* amtEdit = new AmountEdit(m_model->rowInfo().at(_index.row()).currency()->precision(), _parent);
    amtEdit->setMinimum(0);
    amtEdit->setFocus();
    return amtEdit;
  }
...
void MultiInvestmentEntryDelegate::setEditorData(QWidget* _editor, const QModelIndex& _index) const
{
  switch (_index.column())
  {
  case MultiInvestmentModel::COLUMN_SHARES:
  case MultiInvestmentModel::COLUMN_PRICE_PER_SHARE:
  case MultiInvestmentModel::COLUMN_AMOUNT:
    static_cast<AmountEdit*>(_editor)
        ->setAmount(Amount::fromUserLocale(_index.model()->data(_index, Qt::EditRole).toString()));
    break;
...
void MultiInvestmentEntryDelegate::setModelData(QWidget* _editor, QAbstractItemModel* _model,
                                                const QModelIndex& _index) const
{
  switch (_index.column())
  {
  ...
  case MultiInvestmentModel::COLUMN_PRICE_PER_SHARE:
  case MultiInvestmentModel::COLUMN_AMOUNT:
    _model->setData(_index, static_cast<AmountEdit*>(_editor)->amount().toString(), Qt::EditRole);
    break;
...
void MultiInvestmentEntryDelegate::updateEditorGeometry(QWidget* _editor, const QStyleOptionViewItem& _option,
                                                        const QModelIndex& _index) const
{
  Q_UNUSED(_index)
