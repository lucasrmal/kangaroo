...
namespace KLib
{
    ...
    bool accountSort(Account* a, Account* b)
    {
        ...
        {
            return Account::accountTypeWeight(a->type()) < Account::accountTypeWeight(b->type());
        }
    }

    bool accountNodeSort(SimpleAccountController::AccountNode* a, SimpleAccountController::AccountNode* b)
    {
        return accountSort(a->account, b->account);
    }

    bool accountNodeEqual(SimpleAccountController::AccountNode* a, SimpleAccountController::AccountNode* b)
    {
    ...
    SimpleAccountController::SimpleAccountController(QObject *_parent) :
        ...
        m_showClosedAccounts(false)
    {
        loadData();

        connect(LedgerManager::instance(), &LedgerManager::balanceTodayChanged,
                this, &SimpleAccountController::onBalanceTodayChanged);

        connect(Account::getTopLevel(), &Account::accountAdded, this, &SimpleAccountController::onAccountAdded);
        connect(Account::getTopLevel(), &Account::accountModified, this, &SimpleAccountController::onAccountModified);
        connect(Account::getTopLevel(), &Account::accountRemoved, this, &SimpleAccountController::onAccountRemoved);
    }

    QVariant SimpleAccountController::data(const QModelIndex& _index, int _role) const
    {
        if (_index.isValid() && _index.internalPointer())
        {
            return static_cast<AccountNode*>(_index.internalPointer())->data(_index.column(), _role);
        }
        else
        {
            return QVariant();
    ...
    SimpleAccountController::AccountNode::AccountNode(Account* _account, AccountNode* _parent) :
        parent(_parent),
        account(_account),
    ...
    QVariant SimpleAccountController::AccountNode::data(int _column, int _role) const
    {
        switch (_role)
        {
        ...
        case Qt::DisplayRole:
            ...
            {
                return account ? /*account->idSecurity() == Constants::NO_ID ?*/ account->formatAmount(account->balanceToday())
        ...
        case Qt::TextColorRole:
            if (_column == (int)SimpleAccountControllerColumn::BALANCE
                && ((account && account->balanceToday() < 0)
                    || (category != AccountClassification::INVALID && categoryBalance < 0)))
            {
...
//            }
    ...
    void SimpleAccountController::AccountNode::updateTotal()
    {
        if (category != AccountClassification::INVALID)
        {
            ...
            for (AccountNode* n : children)
            {
                ...
                if (bal != 0)
                {
                    if (n->account && !n->account->mainCurrency().isEmpty()) //Currency account
                    {
                        addBy = bal * PriceManager::instance()->rate(n->account->mainCurrency(),
                                                                     topCurrency,
                                                                     today);
                    }
                    else if (n->account) //Security-based account
                    {
                        addBy = bal * PriceManager::instance()->rate(n->account->idSecurity(),
                                                                     topCurrency,
                                                                     today);
                    }
    ...
    QVariant SimpleAccountController::headerData(int _section, Qt::Orientation _orientation, int _role) const
    {
    ...
    QModelIndex SimpleAccountController::index(int _row, int _column, const QModelIndex& _parent) const
    {
    ...
    bool SimpleAccountController::indexIsAfterTypeChange(const QModelIndex& _index) const
    {
        if (_index.isValid() && _index.row() > 0)
        {
            ...
            {
                return node->parent->children[_index.row()-1]->account->type() != node->account->type();
            }
        }
        ...
        if (_account)
        {
            ...
            for (AccountNode* cat : m_root->children)
            {
                for (auto i = cat->children.begin(); i != cat->children.end(); ++i)
                {
                    ...
                    {
                        return createIndex(i - cat->children.begin(),
                                           0,
                                           *i);
                    }
                }
    ...
    QModelIndex SimpleAccountController::indexForCategory(AccountClassification _cat) const
    {
        if (_cat != AccountClassification::INVALID)
        {
            for (auto i = m_root->children.begin(); i != m_root->children.end(); ++i)
            {
                if ((*i)->category == _cat)
                {
                    return createIndex(i - m_root->children.begin(),
                                       0,
                                       *i);
                }
            }
    ...
    void SimpleAccountController::onAccountAdded(KLib::Account* a)
    {
        ...
        if (c == AccountClassification::INVALID || a->isPlaceholder() || (!a->isOpen() && !m_showClosedAccounts))
            return;
        ...
        for (AccountNode* cat : m_root->children)
        {
            if (cat->category == c)
            {
                ...
                auto pos = std::upper_bound(cat->children.begin(), cat->children.end(), n, accountNodeSort);
    ...
    void SimpleAccountController::onAccountRemoved(KLib::Account* a)
    {
        ...
        for (AccountNode* cat : m_root->children)
        {
            ...
            if (pos != cat->children.end())
            {
                ...
                beginRemoveRows(createIndex(m_root->children.indexOf(cat), 0, cat),
                                row, row);
    ...
    void SimpleAccountController::onAccountModified(KLib::Account* a)
    {
        ...
        if (c == AccountClassification::INVALID
            ...
            || a->isPlaceholder())
        {
            //Maybe it was somewhere else before (or it was closed)... try to remove it!
            onAccountRemoved(a);
            return;
        }
        ...
        for (AccountNode* cat : m_root->children)
        {
            if (cat->category == c)
            {
                //Check if the account is indeed in this category. If not, remove it and add it in the correct category.
                AccountNode n(a, cat);
                ...
                if (pos == cat->children.end())
                {
                    //We need to remove it from wherever it is (if it's somewhere), and add it at the correct location.
                    onAccountRemoved(a);
                    onAccountAdded(a);
                }
                else if (pos != cat->children.end() && !a->isOpen() && !m_showClosedAccounts)
                {
                    onAccountRemoved(a);
                }
                else //Check if it is in the correct location!
                {
                    auto range = std::equal_range(cat->children.begin(), cat->children.end(), &n, accountNodeSort);
                    ...
                    else //We need to take the account and put it in the correct range!
                    {
                        QModelIndex parentIndex = createIndex(m_root->children.indexOf(cat),0, cat);
                        int destRow = range.first - cat->children.begin();
                        beginMoveRows(parentIndex, curRow, curRow,
                                      parentIndex, destRow);
                        //If adding after the cur row, take in account that curRow will have been removed before the insert
    ...
    void SimpleAccountController::onBalanceTodayChanged(int _idAccount, const Balances& _difference)
    {
