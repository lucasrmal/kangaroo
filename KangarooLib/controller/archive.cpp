...
namespace KLib
{
    ...
    void Archive::writeFile(const QString& _name, std::function<void(QIODevice*)> _writer)
    {
    ...
    bool Archive::fileExists(const QString& _name) const
    {
        ...
        return m_file->getFileNameList().contains(m_currentDir.isEmpty() ? _name : m_currentDir + "/" + _name);
    }
    ...
    void Archive::openArchive(const QString& _path, bool _writeMode)
    {
        ...
        if (!m_file->open(_writeMode ? QuaZip::mdCreate : QuaZip::mdUnzip))
        {
            delete m_file;
            m_file = nullptr;
            throw IOException(QObject::tr("Unable to open the file. It may not be a valid Kangaroo"));
        }
    }
    ...
    void Archive::setCurrentDirectory(const QString& _dir)
    {
        m_currentDir = _dir;
    }
    ...
    void Archive::loadFiles(IFileManager* _manager)
    {
        ...
        for (QString filename : m_file->getFileNameList())
        {
            if (filename.startsWith(m_currentDir + "/"))
            {
                ...
                {
                    qDebug() << QObject::tr("Unable to open file %1 in archive.").arg(filename);
                }
            }
        }
    }
    ...
    QIODevice* Archive::openMainFile(bool _writeMode)
    {
        ...
        {
            if (!fileExists(MAIN_FILE_NAME) || !m_file->setCurrentFile(MAIN_FILE_NAME))
            {
