...
namespace KLib {
    ...
    AccountController::AccountController(bool _openAccountsOnly, QObject* _parent) :
        AccountController(Account::getTopLevel(), _openAccountsOnly, AccountTypeFlags::Flag_All, _parent)
    {
    }

    AccountController::AccountController(Account* _topLevel, bool _openAccountsOnly, int _flags, QObject *_parent) :
        ...
        m_endDate(QDate::currentDate())
    {
        ...
        connect(m_topLevel, &Account::accountAdded,    this, &AccountController::onAccountAdded);
        connect(m_topLevel, &Account::accountModified, this, &AccountController::onAccountModified);
        connect(m_topLevel, &Account::accountRemoved,  this, &AccountController::onAccountRemoved);
    }

    Account* AccountController::accountForIndex(const QModelIndex& _index) const
    {
        return _index.isValid() ? static_cast<AccountNode*>(_index.internalPointer())->account
                                : nullptr;
    }
    ...
    {
        return (_a->isOpen() || !m_openOnly)
                && Account::matchesFlags(_a, m_flags);
    }
    ...
    AccountController::AccountNode* AccountController::AccountNode::find(Account* _a)
    {
        if (account->id() == _a->id())
            return this;
    ...
    void AccountController::setBalancesBetween(const QDate& _start, const QDate& _end)
    {
        m_startDate = _start;
        m_endDate = _end;
    ...
    QVariant AccountController::data(const QModelIndex &index, int role) const
    {
        ...
        Account* account = static_cast<AccountNode*>(index.internalPointer())->account;
        ...
        if (role == Qt::DisplayRole)
        {
            ...
            case AccountColumn::BALANCE_CURRENT:
                return account->isPlaceholder() ? QVariant()
                                                : account->formatAmount(account->balanceBetween(m_startDate,
                                                                                                m_endDate));

            case AccountColumn::VALUE_CURRENT:
                return Account::getTopLevel()->formatAmount(account->treeValueBetween(m_startDate,
                                                                                      m_endDate));
...
//                return Account::getTopLevel()->formatAmount(account->treeValue());
...
//                return account->treeValue().toDouble();
        ...
        else if (role == Qt::DecorationRole && index.column() == AccountColumn::NAME)
        {
    ...
    QVariant AccountController::headerData(int section, Qt::Orientation orientation, int role) const
    {
        ...
        if (role == Qt::DisplayRole)
        {
            ...
            case AccountColumn::VALUE_CURRENT:
                return tr("Current Value (%1)").arg(Account::getTopLevel()->mainCurrency());
...
//            case AccountColumn::VALUE_FUTURE:
//                return tr("Future Value (%1)").arg(Account::getTopLevel()->currency());
    ...
    QModelIndex AccountController::index(int row, int column, const QModelIndex& parent) const
    {
    ...
    void AccountController::onAccountRemoved(Account* a)
    {
        if (a)
        {
            ...
            if (node)
            {
                ...
                int parentRow = theParent->parent ? theParent->parent->children.indexOf(theParent)
                                                  : 0;
    ...
    void AccountController::onAccountModified(Account* a)
    {
        if (a)
        {
            ...
            else if (node)
            {
                ...
                {
                    int row = theParent->children.indexOf(node);
                    emit dataChanged(createIndex(row, 0, theParent),
    ...
    AccountSortFilterProxyModel::AccountSortFilterProxyModel(QObject* _parent)
        : QSortFilterProxyModel(_parent),
          m_includeChildrenInFilter(true)
    {
        setFilterKeyColumn(AccountColumn::NAME);
        setFilterCaseSensitivity(Qt::CaseInsensitive);
    }

    bool AccountSortFilterProxyModel::lessThan(const QModelIndex& _left, const QModelIndex& _right) const
    {
        if (_left.column() == AccountColumn::NAME)
        {
            AccountController::AccountNode* l = static_cast<AccountController::AccountNode*>(_left.internalPointer());
            AccountController::AccountNode* r = static_cast<AccountController::AccountNode*>(_right.internalPointer());
            ...
            {
                return Account::accountTypeWeight(l->account->type()) < Account::accountTypeWeight(r->account->type());
            }
        }
        else
        {
    ...
    bool AccountSortFilterProxyModel::filterAcceptsRow(int source_row, const QModelIndex& source_parent) const
    {
        std::function<bool(const QModelIndex&)> ancestorMatches;
        ancestorMatches = [this, &ancestorMatches] (const QModelIndex& parent) {
            if (!parent.isValid()) {
                return false;
            } else if (sourceModel()->data(parent, filterRole()).toString().contains(filterRegExp())) {
                return true;
            } else {
        ...
        if (!filterRegExp().isEmpty()) {
            ...
            if (source_index.isValid()) {
                if (sourceModel()->data(source_index, filterRole()).toString().contains(filterRegExp())) {
                    return true;
                } else if (m_includeChildrenInFilter && ancestorMatches(source_index.parent())) {
                    return true;
                } else {
                    // if any of its children matches the filter, then current index matches the filter as well
