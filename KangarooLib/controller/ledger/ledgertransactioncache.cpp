...
LedgerTransactionCache::LedgerTransactionCache(LedgerController* _controller)
  ...
  connect(LedgerManager::instance(), &LedgerManager::splitAdded, this,
          &LedgerTransactionCache::onSplitAdded);
  connect(LedgerManager::instance(), &LedgerManager::splitRemoved, this,
          &LedgerTransactionCache::onSplitRemoved);
  connect(LedgerManager::instance(), &LedgerManager::splitAmountChanged, this,
          &LedgerTransactionCache::onSplitAmountChanged);
  connect(LedgerManager::instance(), &LedgerManager::transactionDateChanged,
          this, &LedgerTransactionCache::onTransactionDateChanged);

  connect(ScheduleManager::instance(), &ScheduleManager::scheduleAdded, this,
          &LedgerTransactionCache::onScheduleAdded);
  connect(ScheduleManager::instance(), &ScheduleManager::scheduleRemoved, this,
          &LedgerTransactionCache::onScheduleRemoved);
  connect(ScheduleManager::instance(), &ScheduleManager::scheduleModified, this,
          &LedgerTransactionCache::onScheduleModified);
  connect(ScheduleManager::instance(),
          &ScheduleManager::scheduleOccurrenceCanceled, this,
          &LedgerTransactionCache::onScheduleOccurrenceEnteredOrCanceled);
  connect(ScheduleManager::instance(),
          &ScheduleManager::scheduleOccurrenceEntered, this,
          &LedgerTransactionCache::onScheduleOccurrenceEnteredOrCanceled);
}
...
bool LedgerTransactionCache::compareItems(const_reference _first,
                                          const_reference _second,
                                          const CacheItem* _item,
  ...
  if (firstDate != secondDate) {
    ...
    return true;
  } else {  // If both are schedules/not schedules. Order from largest to
            // smallest  amounts
    Amount first = Transaction::totalForAccount(m_controller->account()->id(),
                                                _first.transaction()->splits());
    Amount second = Transaction::totalForAccount(
        m_controller->account()->id(), _second.transaction()->splits());
    if (first == 0) {
      return false;
    } else if (second == 0) {
      return true;
...
void LedgerTransactionCache::reloadData() {
  ...
  m_displayPolicy =
      ScheduleDisplayPolicy(SettingsManager::instance()
                                ->value("Ledger/ScheduleDisplayPolicy")
                                .toInt());

  if (m_displayPolicy == ScheduleDisplayPolicy::DaysInFuture) {
    m_displayPolicyCount = SettingsManager::instance()
                               ->value("Ledger/ScheduleDisplayDays")
                               .toInt();
  } else {
    m_displayPolicyCount = SettingsManager::instance()
                               ->value("Ledger/ScheduleDisplayInstances")
                               .toInt();
  }
  ...
  if (m_displayPolicy != ScheduleDisplayPolicy::NoSchedules) {
    QList<Schedule*> schedules = ScheduleManager::instance()->schedulesFor(
        m_controller->account()->id());
  ...
  // For each day, we want to show, in this order:
  // DEBITS, CREDITS, BANNERS, SCHEDULES
  ...
  for (auto it = transactions->begin(); it != transactions->end(); ++it, ++i) {
    ...
    //    bool before =
    //    Transaction::totalForAccount(m_controller->account()->id(),
    //                                               (*it)->splits()) > 0;
...
void LedgerTransactionCache::reloadBalanceAndIndexFrom(int _from) {
  for (int i = std::max(0, _from); i < m_cache.size(); ++i) {
    // Update the balance
    m_cache[i].runningBalance =
        i > 0 ? m_cache[i - 1].runningBalance
              : Balances();  // Balance before the current row
...
int LedgerTransactionCache::addItem(const CacheItem& _item, bool _reloadIndex) {
  ...
  auto i = std::upper_bound(m_cache.begin(), m_cache.end(), _item,
                            std::bind(&LedgerTransactionCache::compareItems,
                                      this, _1, _2, nullptr, QDate()));
  ...
  if (!m_filterIsApplied) {
    ...
    if (m_filterFunction(_item)) {
      i = std::upper_bound(m_filteredSortedCache.begin(),
                           m_filteredSortedCache.end(), _item,
                           std::bind(&LedgerTransactionCache::compareItems,
                                     this, _1, _2, nullptr, QDate()));
...
void LedgerTransactionCache::removeRow(int _row, bool _reloadIndex) {
  ...
  // See if the row is also in the filter, in which case we remove it there
  // instead
...
void LedgerTransactionCache::replaceItemAt(int _row, const QDate& _oldDate,
                                           bool _reloadIndex) {
  using namespace std::placeholders;
  ...
  auto i = std::upper_bound(
      m_cache.begin(), m_cache.end(), CacheItem(m_cache[_row]),
  ...
  // Check if the search was accurate (may be off a bit if the transaction
  // amount has changed from debit to credit or vice-versa). We just check if in
  // order, otherwise go up and down as required.
  //        while (newRow != 0 && !compareItems(m_cache[newRow-1],
  //        m_cache[_row]))
  //        {
  //            --newRow;
  //        }
  //        while (newRow < m_cache.count()-1 && !compareItems(m_cache[_row],
  //        m_cache[newRow+1]))
  //        {
  //            ++newRow;
  //        }
  ...
  if (newRow != _row && newRow != _row + 1)  // No no-op moves
  {
    ...
    // std::swap(m_cache[_row].cachedSubRowCount,
    // m_cache[newRow].cachedSubRowCount);
    ...
    m_cache.move(
        _row, _row < newRow
                  ? newRow - 1
                  : newRow);  // When inserting back, does as if the "removed"
    ...
    // Update the balance
    emit m_controller->dataChanged(
        m_controller->index(reloadFrom, m_controller->col_balance()),
        m_controller->index(m_controller->rowCount(),
                            m_controller->col_balance()));
  } else {
    updateRow(_row);
...
void LedgerTransactionCache::updateRow(int _row) {
  ...
  if (cachedSubRowCount(_row) < m_controller->cacheSubRowCount(_row)) {
    ...
    m_controller->beginInsertRows(parent, cachedSubRowCount(_row),
    ...
    emit m_controller->dataChanged(
        parent, m_controller->index(
                    /* row    */ m_controller->cacheSubRowCount(_row) - 1,
    ...
    m_controller->beginRemoveRows(parent, m_controller->cacheSubRowCount(_row),
    ...
    emit m_controller->dataChanged(
        parent, m_controller->index(
                    ...
                    /* column */ m_controller->columnCount() - 1,
                    /* parent */ parent));
  } else if (m_controller->cacheSubRowCount(
                 _row))  // If there is sub rows, reload while including them
  {
    emit m_controller->dataChanged(
        parent, m_controller->index(
                    /* row    */ m_controller->cacheSubRowCount(_row) - 1,
...
QList<QDate> LedgerTransactionCache::nextOccurrences(
    const Schedule* _schedule) const {
  switch (m_displayPolicy) {
    ...
    case ScheduleDisplayPolicy::DaysInFuture:
      return _schedule->nextOccurrencesDates(
          -1, QDate::currentDate().addDays(m_displayPolicyCount));
...
void LedgerTransactionCache::onSplitAdded(const Transaction::Split& _split,
                                          Transaction* _tr) {
...
void LedgerTransactionCache::onSplitRemoved(const Transaction::Split& _split,
                                            Transaction* _tr) {
  if (m_transactionIndex.contains(_tr->id())) {
    if (_tr->relatedTo(m_controller->account()->id())) {
      updateRow(m_transactionIndex[_tr->id()]);
...
void LedgerTransactionCache::onSplitAmountChanged(
    const Transaction::Split& _split, Transaction* _tr) {
  if (m_transactionIndex.contains(_tr->id())) {
    if (_split.idAccount == m_controller->account()->id()) {
...
void LedgerTransactionCache::onTransactionDateChanged(Transaction* _tr,
                                                      const QDate& _old) {
  if (m_transactionIndex.contains(_tr->id())) {
...
void LedgerTransactionCache::onScheduleAdded(Schedule* s) {
  ...
  //&& m_controller->canEditTransaction(s->transaction()))
  {
    ...
    for (const QDate& d : next) {
      min_row = std::min(
          min_row,
          addItem(CacheItem(s, d, m_controller->subRowCount(s->transaction())),
                  false));
    }
  ...
  if (schedule_iter != m_scheduleIndex.end()) {
    if (schedule_iter->count()) {
      ...
      for (auto i = schedule_iter->end() - 1;; --i) {
        // Reload balance and cache only if smallest row, otherwise waste of
        // time The index entry will be removed by removeRow once it removes the
        // last occurrence of the schedule.
        bool isLast =
            i ==
            schedule_iter->begin();  // Compute this before, as the iterator
                                     // will become invalid if it's the last
                                     // (whole QMap will be deleted)
...
void LedgerTransactionCache::onScheduleModified(Schedule* s) {
  ...
  if (schedule_iter == m_scheduleIndex.end()) {
    onScheduleAdded(s);  // Try to add it. This will check if the schedule
                         // relates to this ledger or not.
  } else if (!s->transaction()->relatedTo(m_controller->account()->id()))
  // || !m_controller->canEditTransaction(s->transaction()))
  {
    // The schedule has changed such as it does not relates to this ledger
    // anymore - remove it.
    onScheduleRemoved(s);
  } else {
    ...
    while (i_cur != next.end() || i_prev != schedule_iter->end()) {
      if (i_prev == schedule_iter->end() ||
          (i_cur != next.end() && *i_cur < i_prev.key()))  // New, add
      {
        add << *i_cur;
        ++i_cur;
    ...
    reloadBalanceAndIndexFrom(
        min_row);  // Nothing will happen if we did not move rows around.
  }
}

void LedgerTransactionCache::onScheduleOccurrenceEnteredOrCanceled(
    Schedule* s, const QDate& _instanceDate) {
  // Check if we have it somewhere
  if (m_scheduleIndex.contains(s->id()) &&
    ...
    // Check if the display policy is the number of instances, in which case we
    // may need to show more instances.
