...
namespace KLib
{
    ...
    LedgerController::LedgerController(Ledger* _ledger, LedgerBuffer* _buffer, QObject* _parent)
            : QAbstractItemModel(_parent),
              ...
              m_cache(this)
    {
        m_dateFormat                = SettingsManager::instance()->value("General/DateFormat").toString();
        m_accountHeightDisplayed    = SettingsManager::instance()->value("Ledger/AccountHeightDisplayed").toInt();
        m_askBeforeAddingNewPayee   = SettingsManager::instance()->value("Ledger/AskBeforeAddingNewPayee").toInt();
        m_expandAllSplits           = true;
        m_defaultFont.setPointSize(SettingsManager::instance()->value("Ledger/FontSize").toInt());

        m_precision = account()->idSecurity() == Constants::NO_ID
                                                 ? CurrencyManager::instance()->get(account()->mainCurrency())->precision()
                                                 : SecurityManager::instance()->get(account()->idSecurity())->precision();

        connect(m_buffer, &LedgerBuffer::rowInserted, this, &LedgerController::onBufferRowInserted);
        connect(m_buffer, &LedgerBuffer::rowRemoved,  this, &LedgerController::onBufferRowRemoved);
        connect(m_buffer, &LedgerBuffer::showMessage, this, &LedgerController::showMessage);
    }
    ...
    LedgerWidgetDelegate* LedgerController::buildDelegate(LedgerWidget* _widget) const
    {
        return new LedgerWidgetDelegate(this, _widget);
    }

    Account* LedgerController::account() const
    {
        return m_ledger->account();
    }

    Ledger* LedgerController::ledger() const
    {
        return m_ledger;
    }
    ...
    bool LedgerController::duplicateTransaction(const QModelIndex& _index, const QDate& _date)
    {
    ...
    void LedgerController::editNote(const QModelIndex& _index)
    {
        ...
        if (row == -1 || (hasModifiedRow() && row != modifiedRow()))
            return;
        ...
        CAMSEGDialog* dlg = new CAMSEGDialog(txtEdit,
                                             CAMSEGDialog::DialogWithPicture,
                                             CAMSEGDialog::OkCancelButtons,
                                             m_parentWidget);
        ...
        if (dlg->exec() == QDialog::Accepted)
        {
            try
            {
                ...
                {
                    cacheItemAtRow(row).schedule->transaction()->setNote(txtEdit->toPlainText());
                }
                else
                {
                    cacheItemAtRow(row).editableTransaction()->setNote(txtEdit->toPlainText());
                }
            }
            catch (ModelException e)
            {
    ...
    void LedgerController::editAttachments(const QModelIndex& _index)
    {
        ...
        FormEditAttachments* dlg = new FormEditAttachments(attachments, m_parentWidget);
        dlg->exec();
        ...
        try
        {
            ...
            {
                cacheItemAtRow(row).schedule->transaction()->setAttachments(attachments.toSet());
            }
            else
            {
                cacheItemAtRow(row).editableTransaction()->setAttachments(attachments.toSet());
            }
        }
        catch (ModelException e)
        {
            QMessageBox::warning(m_parentWidget,
                                 tr("Transaction Attachments"),
    ...
    bool LedgerController::schedule(const QModelIndex& _index, const QDate& _date)
    {
        ...
        if (hasModifiedRow())
        {
            ...
            else
            {
                ...
                if (form.exec() == QDialog::Accepted
                    && (row == newTransactionRow() || m_buffer->schedule.recurrence().frequency == Frequency::Once))
                {
        ...
        else if (row != newTransactionRow() && row >= 0 && row < rowCount())
        {
            ...
            else if (duplicateTransaction(_index, _date)) //Duplicate the transaction as schedule
            {
    ...
    Amount LedgerController::totalForAccount(int _row) const
    {
        ...
        {
            tot = Transaction::totalForAccount(account()->id(), m_cache[_row].transaction()->splits());
        }
        else
        {
            return 0;
    ...
    int LedgerController::newTransactionRow() const
    {
    ...
    bool LedgerController::submitChanges(int &_firstErrorColumn)
    {
        ...
        if (!errors.isEmpty())
        {
            ...
            int ans = QMessageBox::question(m_parentWidget,
                                            tr("Save Changes"),
        ...
        try
        {
            //Update the number of rows to insert/remove the right number of rows later.
    ...
    void LedgerController::discardChanges()
    {
        ...
        if (row != newTransactionRow())
        {
            ...
            emit dataChanged(parent, index(cacheSubRowCount(row)-1, columnCount()-1, parent));
        }
        else
        {
            if (rowCount > 1)
    ...
    {
        return _row != -1
               && (_row == newTransactionRow() || canEditTransaction(m_cache[_row].transaction(), _message));
    }

    QList<Transaction::Split> LedgerController::displayedSplits(const Transaction* _tr) const
    {
    ...
    QVariant LedgerController::cacheData(int _column, int _cacheRow, int _row, bool _editRole) const
    {
        ...
        {
            QString path = Account::getTopLevel()->getPath(idAccount,
                                                           accountHeightDisplayed());
        ...
        if (_row == 0)
        {
            ...
            {
                return tr->idPayee() == Constants::NO_ID ? QVariant()
        ...
        if (_row == 0 && _column == col_balance()
            && (!m_cache[_cacheRow].schedule || m_cache[_cacheRow].dueDate > QDate::currentDate()))
        {
        ...
        else if (_row == 0 && !isSplit)
        {
            if (_column == col_transfer())
            {
                ...
                {
                    return QVariant::fromValue(AccountCurrency(splits[1].idAccount, splits[1].currency));
                }
            }
            else if (_column == col_debit())
            {
                return splits[0].amount > 0 ? formatCurrency(splits[0].amount, splits[0].currency) : QVariant();
            }
        ...
        else if (isSplit && _row < tr->splitCount())
        {
            if (_column == col_transfer())
            {
                ...
                {
                    return QVariant::fromValue(AccountCurrency(splits[_row].idAccount, splits[_row].currency));
                }
            }
            else if (_column == col_debit())
            {
                return splits[_row].amount > 0 ? formatCurrency(splits[_row].amount, splits[_row].currency)
                                                  : QVariant();
            }
            else if (_column == col_credit())
            {
    ...
    QString LedgerController::formatCurrency(const Amount& _amount, const QString& _currency) const
    {
...
//        Balances prev = _cacheRow > 0 ? m_balanceCache[_cacheRow-1] : Balances();

//        for (auto i = cur.begin(); i != cur.end(); ++i)
//        {
//            if (!prev.contains(i.key()) || prev.value(i.key()) != i.value()) //Show it!
//            {
//                show += CurrencyManager::instance()->get(i.key())->formatAmount(i.value()) + " ";
//            }
//        }
...
//        return show;
        ...
        return account()->formatAmount(m_cache.balanceAt(_cacheRow).inCurrency(account()->mainCurrency()));
    }
    ...
    int LedgerController::subRowCount(const Transaction* _tr) const
    {
        ...
        return m_expandAllSplits && count > 2
                ? count-1
                : 0;
    }
    ...
    int LedgerController::rowCount(const QModelIndex& _parent) const
    {
        ...
        {
            return m_cache.count() + 1; //One more for the ADD row
        }
        else if (!_parent.parent().isValid() //============================== Cache
                 && _parent.row() == m_buffer->row)
        {
            return m_buffer->rowCount()-1;
        }
        else if (!_parent.parent().isValid()  //============================= Buffer transaction
                 && _parent.row() >= 0 && _parent.row() < m_cache.count())
        {
            return cacheSubRowCount(_parent.row());
        }
        else
        {
            return 0;
    ...
    QVariant LedgerController::data(const QModelIndex& _index, int _role) const
    {
        ...
        if (!_index.isValid() || mainRow >= rowCount() || _index.column() >= columnCount())
            return QVariant();
        ...
        try
        {
            ...
            case Qt::TextColorRole:
                if (mainRow != newTransactionRow())
                {
                    ...
                    {
                        return m_cache.balanceAt(mainRow) < 0 ? QColor(Qt::red) : QColor(Qt::black);
                    }
                }
                break;
            case Qt::FontRole:
            ...
            case Qt::ToolTipRole:
                if (_index.column() == col_status())
                {
                             ...
                             && m_cache[mainRow].schedule)
                    {
                        ...
                        {
                            return m_cache[mainRow].schedule->remindBefore() < 0 ? tr("Overdue Scheduled Transaction")
                                                                                 : tr("Overdue Bill");
                        }
                        else if (mainRow != newTransactionRow()
                                 && m_cache[mainRow].schedule)
                        {
                            return m_cache[mainRow].schedule->remindBefore() < 0 ? tr("Scheduled Transaction")
                                                                                 : tr("Bill Reminder");
                        }
                    }
                    else
            ...
            case Qt::DisplayRole:
                ...
                {
                    return m_buffer->data(/* column */ _index.column(),
                ...
                {
                    return cacheData(/* column    */ _index.column(),
                                     /* cache row */ mainRow,
    ...
    QVariant LedgerController::headerData(int _section, Qt::Orientation _orientation, int _role) const
    {
    ...
    QModelIndex LedgerController::index(int _row, int _column, const QModelIndex& _parent) const
    {
    ...
    bool LedgerController::startEdit(int _row)
    {
        ...
        //Check that we do not want to edit a different row than the one currently edited
    ...
    bool LedgerController::canRemoveSplitAt(const QModelIndex& _index)
    {
        return _index.isValid()
                && mapToCacheRow(_index) == modifiedRow()
    ...
    bool LedgerController::setData(const QModelIndex& _index, const QVariant& _value, int _role)
    {
        const int mainRow = mapToCacheRow(_index);
        ...
        if (_index.isValid()
            ...
            && startEdit(mainRow))
        {
            bool ret =  m_buffer->setData(/* column */ _index.column(),
                                          /* row    */ _index.parent().isValid() ? _index.row()+1 : 0,
                                          /* value  */ _value,
                                          /* cntlr  */ this);
    ...
    void LedgerBuffer::showImbalances(const QList<Transaction::Split>& splits)
    {
        ...
        emit showMessage(QObject::tr("Imbalances:%1")
                         .arg(Transaction::splitsImbalances(splits)));
    }
    ...
    void LedgerBuffer::clear()
    {
        ...
        schedule.setRecurrence(Recurrence(LedgerWidgetDelegate::lastUsedDate(), Frequency::Once, 0));
        attachments.clear();
    }
    ...
    bool LedgerBuffer::setData(int _column, int _row, const QVariant& _value, LedgerController* _controller)
    {
        ...
        if (_row == 0)
        {
            ...
            else if (_column == _controller->col_date())
            {
                ...
                if (isSchedule
                    && (schedule.recurrence().frequency == Frequency::Once
        ...
        if (splits.count() && _row < splits.count()) //If we're in split mode
        {
            ...
            else if (_column == _controller->col_debit())
            {
                ...
                if (a == 0 && splits[_row].amount < 0) // Check if there was data in the Credit column
                {
                    return false;
                }
            ...
            else if (_column == _controller->col_credit())
            {
                ...
                if (a == 0 && splits[_row].amount > 0) // Check if there was data in the Debit column
                {
                    return false;
                }
    ...
    QVariant LedgerBuffer::data(int _column, int _row, bool _editRole, const LedgerController* _controller) const
    {
        ...
        if (splits.count() && _row < splits.count())
        {
            ...
            {
                return _controller->formatCurrency(splits[_row].amount, splits[_row].currency);
            }
            else if (_column == _controller->col_credit()
                     && splits[_row].amount < 0)
            {
        ...
        {
            QString cur = _controller->account()->allCurrencies().contains(transferCurrency) ? transferCurrency
            ...
            if (_column == _controller->col_transfer())
            {
                ...
                {
                    return QVariant::fromValue(AccountCurrency(idTransfer, transferCurrency));
                }
            }
            else if (_column == _controller->col_debit())
            {
    ...
    void LedgerBuffer::load(const Schedule* _schedule, const LedgerController* _controller)
    {
    ...
    void LedgerBuffer::load(const Transaction* _transaction, const LedgerController* _controller)
    {
        no   = _transaction->no();
        ...
        try
        {
            payee = _transaction->idPayee() == Constants::NO_ID ? QString()
    ...
    void LedgerBuffer::loadSplits(const QList<Transaction::Split>& _splits, const LedgerController* _controller)
    {
        auto loadFromSplit = [this] (const Transaction::Split& cur, const Transaction::Split& oth)
        {
        ...
        else if (Transaction::isCurrencyExchange(_splits))
        {
            ...
            for (const Transaction::Split& s : _splits)
            {
                ...
                else if (Account::getTopLevel()->getChild(s.idAccount)->type() != AccountType::TRADING)
                {
                    oth = s;
                }
            }
        ...
        else
        {
            ...
            Amount tot = Transaction::totalForAccount(_controller->account()->id(), splits);
    ...
    QStringList LedgerBuffer::validate(int &_firstErrorColumn, const LedgerController* _controller)
    {
        QStringList errors;
        _firstErrorColumn = -1;
        ...
        {
            errors << QObject::tr("The schedule is invalid. Click on the <emph>Schedule</emph> button to edit it.");
        }
        ...
        else if (idTransfer == Constants::NO_ID) //Check if a transfer account is set
        {
    ...
    QList<Transaction::Split> LedgerBuffer::splitsForSaving(LedgerController* _controller, bool &_ok) const
    {
        //--------------------Amount--------------------
        Amount total;
        ...
        if (splits.isEmpty())
        {
                ...
                && !_controller->account()->allCurrencies().contains(transferCurrency))
            {
                ...
                FormCurrencyExchange frm(from, to, total, date, _controller->parentWidget());

                if (row != _controller->newTransactionRow()) // Transaction is being modified
                {
                ...
                tmp_splits << Transaction::Split(total, _controller->account()->id(), from);
                tmp_splits << Transaction::Split(-frm.exchangedAmount(), idTransfer, to);
                Transaction::addTradingSplits(tmp_splits);

//                tmp_splits << Transaction::Split(-total, CurrencyManager::instance()->get(from)->tradingAccount()->id());
//                tmp_splits << Transaction::Split(frm.exchangedAmount(), CurrencyManager::instance()->get(to)->tradingAccount()->id());

            }
            else //No exchange, a plain 2 account transfer
            {
                tmp_splits << Transaction::Split(total, _controller->account()->id(), _controller->account()->mainCurrency());
    ...
    QHash<QString, QVariant> LedgerBuffer::propertiesForSaving(LedgerController* _controller) const
    {
        Q_UNUSED(_controller)
        return QHash<QString, QVariant>();
    }

    bool LedgerBuffer::saveToTransaction(Transaction* _transaction, LedgerController* _controller)
    {
        ...
        if (!payee.isEmpty())
        {
            ...
            catch (ModelException)
            {
                ...
                {
                    int ans = QMessageBox::question(_controller->parentWidget(),
    ...
    {
        //Need to clear row before inserting, since the new transaction will trigger a reload of that row using the
        ...
        try
        {
            ...
            else //Modification
            {
                if (isSchedule)
                {
                    ...
                    QStringList options;
                    options << tr("Post this occurrence with the current changes, do not modify the schedule")
                            << tr("Post this occurrence with the current changes and apply them to the schedule as well")
                            << tr("Do not post this occurrence, but save the current changes to the schedule");


                    int option = OptionsDialog::askForOptions(Core::pixmap("edit"),
                                                              tr("Edit Schedule"),
                ...
                {
                    transaction = _controller->cacheItemAtRow(tmp_row).editableTransaction();
                    row = NO_ROW;
                ...
                emit _controller->dataChanged(_controller->index(tmp_row, 0),
        ...
        {
            //Do not need to delete the transaction if exception as will be done by the Ledger.
            row = tmp_row;
            throw;
        }
    ...
    Amount LedgerBuffer::totalForAccount(const LedgerController* _controller) const
    {
        if (splits.isEmpty())
        {
    ...
    void LedgerBuffer::changeToSplitTransaction(const LedgerController* _controller)
    {
        if (splits.count())
            return;
        ...
        Transaction::Split s2(debit > 0 ? -debit : credit,
                              idTransfer,
                              idTransfer != Constants::NO_ID ? Account::getTopLevel()->account(idTransfer)->mainCurrency()
                                                             : "");
                                     ...
                                     const QModelIndex& _index) const
    {
        ...
        const LedgerController* con = qobject_cast<const LedgerController*>(_index.model());
        ...
        if (con)
        {
            ...
            else
            {
                ...
                int row = _index.parent().isValid() ? _index.parent().row()
                                                    : _index.row();
...
//                                         Qt::KeepAspectRatio);
                ...
                _painter->drawPixmap(newOption.rect.left() + (newOption.rect.width() - toPaint.width())/2,
                                     newOption.rect.top() + (newOption.rect.height() - toPaint.height())/2,
                                     toPaint.width(),
                                     toPaint.height(),
                                     toPaint);
                _painter->restore();
...
//        }
        ...
        if (_index.column() == _index.model()->columnCount()-1) //Last column, draw right vertical line
        {
            _painter->drawLine(borderRect.topRight(), borderRect.bottomRight());
        }
...
//        }
    ...
    QSize LedgerWidgetDelegate::sizeHint(const QStyleOptionViewItem& _option, const QModelIndex& _index) const
    {
        return QSize(QStyledItemDelegate::sizeHint(_option, _index).width(), m_rowHeight);
    }
                                                ...
                                                const QModelIndex& _index) const
    {
      ...
      {
        const Account* a = static_cast<const LedgerController*>(_index.model())->account();
      ...
      {
        setCurrentEditor(QStyledItemDelegate::createEditor(_parent, _option, _index));
      }

      return m_currentEditor;
    }

    void LedgerWidgetDelegate::setEditorData(QWidget* _editor, const QModelIndex& _index) const
    {
        if (_index.column() == m_controller->col_date())
        {
        ...
        else if (_index.column() == m_controller->col_payee())
        {
            ...
            if (!payee.isEmpty())
            {
                ...
                if (idx != -1)
                    cboPayee->setCurrentIndex(idx);
            }
        }
        else if (_index.column() == m_controller->col_transfer())
        {
            AccountCurrency ac = _index.model()->data(_index, Qt::EditRole).value<AccountCurrency>();
                                            ...
                                            const QModelIndex& _index) const
    {
        ...
        else if (_index.column() == m_controller->col_transfer())
        {
            ...
            AccountCurrency c(a ? a->id() : Constants::NO_ID, accSelect->currentCurrency());

            _model->setData(_index, QVariant::fromValue<AccountCurrency>(c), Qt::EditRole);
        }
        else if (_index.column() == m_controller->col_debit()
                 || _index.column() == m_controller->col_credit())
        {
    ...
    void LedgerWidgetDelegate::updateEditorGeometry(QWidget* _editor,
                                                    const QStyleOptionViewItem& _option,
                                                    const QModelIndex& _index) const
    {
        Q_UNUSED(_index)
    ...
    void LedgerWidgetDelegate::setCurrentEditor(QWidget* _editor) const
    {
        ...
        connect(m_currentEditor, &QObject::destroyed,
                this, &LedgerWidgetDelegate::onEditorDestroyed);
    }
