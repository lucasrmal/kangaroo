...
class LedgerBuffer : public QObject {
 ...
 public:
  ...
  virtual bool setData(int _column, int _row, const QVariant& _value,
                       LedgerController* _controller);
  virtual QVariant data(int _column, int _row, bool _editRole,
                        const LedgerController* _controller) const;

  virtual void load(const Transaction* _transaction,
                    const LedgerController* _controller);
  virtual void load(const Schedule* _schedule,
                    const LedgerController* _controller);

  // virtual void loadFromSplits(const QList<Transaction::Split>& _splits, const
  // LedgerController* _controller);

  virtual QStringList validate(int& _firstErrorColumn,
                               const LedgerController* _controller);

  virtual QList<Transaction::Split> splitsForSaving(
      LedgerController* _controller, bool& _ok) const;
  virtual QHash<QString, QVariant> propertiesForSaving(
      LedgerController* _controller) const;

  /**
   * @brief Saves the changes to the transaction.
   *
   * This transaction may be a newly created transaction, or it may be an
   * existing transaction. It may also be a scheduled transaction.
   *
   * Do not catch ModelExceptions, they will be caught and handled by the
   * LedgerController.
   *
   * @param _transaction
   * @return True if succesfull, False otherwise.
   */
  virtual bool saveToTransaction(Transaction* _transaction,
                                 LedgerController* _controller);
  ...
  /**
   * @brief Saves the buffer to a new/modified transaction. Must clear the
   * buffer before leaving if added succesfully. Do not catch ModelException; it
   * will be caught and handled by LedgerController.
...
class LedgerController : public QAbstractItemModel {
 ...
 protected:
  explicit LedgerController(Ledger* _ledger, LedgerBuffer* _buffer,
 ...
 public:
  ...
  /**
   * @brief This function will be called by LedgerWidget to ask the controller
   * if any extra actions are provided by this controller.
   *
   * Parameters: the LedgerWidger, and a function int, LedgerAction* : first
   * param is the weight, second is the action to add.
   *
   ...
   */
  virtual void addExtraActions(LedgerWidget*,
                               std::function<void(int, LedgerAction*)>) {}
  ...
  virtual bool duplicateTransaction(const QModelIndex& _index,
                                    const QDate& _date = QDate());
   ...
   * @param _date If creating a new schedule, date at which the schedule will
   * start. Otherwise, this is ignored.
   *
   * If in edit and currently edited transaction is a schedule, opens the
   * schedule dialog.
   *
   * Otherwise, if not in edit and if index is a transaction, creates a new
   * schedule from it.
   *
   * Otherwise, if not in edit and if index is a transaction instance, opens the
   * schedule dialog to edit the schedule.
   */
  virtual bool schedule(const QModelIndex& _index,
                        const QDate& _date = QDate());
  ...
  /**
   * @brief Returns first editable index in the same row (main or sub) as
   * _index.
   */
  ...
  const Transaction* transactionAtRow(int _row) const {
    return m_cache[_row].transaction();
  }
  ...
  virtual int col_cleared()
      const = 0;  ///< Cleared status (Cleared/Reconciled/Empty)
 ...
 public slots:
   ...
   *
   * If currently in edit, _index must correspond to the current edited row.
   * Will not do anything if the transaction currently edited is a split.
   *
   * If not currently in edit, starts editing the transaction at _index as a
   * split transaction.
   */
   ...
   *
   * Must not be currently in any other edit. Will start editing a new schedule
 ...
 protected:
  /**
   * @brief Returns a list of all the splits to be displayed for this
   * transaction.
   * @param _tr The transaction
   *
   * The first split <b>MUST</b> be the split related to this transaction.
   *
   * In the base implementation (this class), this method will be used by
   * cacheData to display the splits and by subRowCount to count the number of
   * split rows to display.
   */
  virtual QList<Transaction::Split> displayedSplits(
   ...
   */
  virtual QVariant cacheData(int _column, int _cacheRow, int _row,
   ...
   * @param _cacheRow Row, will ALWAYS be a valid cache row (not
   * newTransactionRow()).
   */
  virtual Balances cacheBalance(int _cacheRow) const;

  /**
   * @brief Checks if the transaction at row _row can be edited by this ledger.
   * Ex: investment transactions cannot be edited by GenericLedgerController.
   * @param _row The row, no need to check, it is valid.
   */
  virtual bool canEditTransaction(const Transaction* _transaction,
                                  QString* _message = nullptr) const = 0;
...
class LedgerWidgetDelegate : public QStyledItemDelegate {
 ...
 protected:
  /**
   * @brief Subclasses that allow other kinds of editors MUST set the current
