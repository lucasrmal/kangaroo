...
namespace KLib {
const QString InvestmentLedgerController::INVALID_TRANSFER_TEXT =
    QObject::tr("<< Invalid! >>");

InvestmentLedgerController::InvestmentLedgerController(Ledger* _ledger,
                                                       QObject* _parent)
    : LedgerController(
          _ledger, new InvestmentLedgerBuffer(_ledger->account()->idSecurity()),
          _parent),
      m_security(
          SecurityManager::instance()->get(m_ledger->account()->idSecurity())),
...
LedgerWidgetDelegate* InvestmentLedgerController::buildDelegate(
    LedgerWidget* _widget) const {
  return new InvestmentLedgerWidgetDelegate(this, _widget);
}

QModelIndex InvestmentLedgerController::firstEditableIndex(
    const QModelIndex& _index) const {
  if (_index.isValid()) {
    return _index.parent().isValid()
               ? index(_index.row(), InvestmentLedgerColumn::TRANSFER,
                       _index.parent())
...
QModelIndex InvestmentLedgerController::lastEditableIndex(
    const QModelIndex& _index) const {
  if (_index.isValid()) {
    QSet<InvestmentAction> allowsPrice = {InvestmentAction::Buy,
    ...
    InvestmentAction ac =
        (InvestmentAction)data(
            index(/* row */ _index.parent().isValid() ? _index.parent().row()
                                                      : _index.row(),
    ...
    return _index.parent().isValid() || !allowsPrice.contains(ac)
               ? index(_index.row(), InvestmentLedgerColumn::QUANTITY,
                       _index.parent())
...
Qt::ItemFlags InvestmentLedgerController::flags(
    const QModelIndex& _index) const {
  ...
  if (_index.column() == col_balance()  // Balance column
      || (_index.parent()
              .isValid()  // Sub-rows can only edit transfer or Quantity.
          && _index.column() != InvestmentLedgerColumn::TRANSFER &&
          _index.column() != InvestmentLedgerColumn::QUANTITY) ||
      (_index.column() ==
           col_status()  // Status, Flag and Cleared cannot be edited
       || _index.column() == col_flag() || _index.column() == col_cleared()) ||
      (_index.column() ==
           InvestmentLedgerColumn::PRICE  // If no price for this action
...
int InvestmentLedgerController::subRowCount(const Transaction* _tr) const {
  const InvestmentTransaction* trans =
...
Balances InvestmentLedgerController::cacheBalance(int _cacheRow) const {
  const InvestmentTransaction* trans =
      qobject_cast<const InvestmentTransaction*>(transactionAtRow(_cacheRow));

  if (trans && trans->action() == InvestmentAction::StockSplit &&
      _cacheRow > 0) {
    Balances b = m_cache.balanceAt(_cacheRow - 1);
    b.add("", InvestmentTransaction::balanceAfterSplit(b.value(""),
                                                       trans->splitFraction()));
...
QVariant InvestmentLedgerController::cacheData(int _column, int _cacheRow,
                                               int _row, bool _editRole) const {
  const InvestmentTransaction* trans =
      qobject_cast<const InvestmentTransaction*>(
          m_cache[_cacheRow].transaction());

  if (!trans) return QVariant();

  auto formatIdAccount = [_editRole, this](const Transaction::Split& s) {
    if (_editRole) {
      return QVariant::fromValue<AccountCurrency>(
          AccountCurrency(s.idAccount, s.currency));
    } else {
      Account* a = Account::getTopLevel()->account(s.idAccount);

      return a ? QVariant(Account::getTopLevel()->getPath(
                     a->id(), this->accountHeightDisplayed()))
  ...
  switch (_column) {
    case InvestmentLedgerColumn::ACTION:
      if (_editRole) {
        ...
        return QString("%1:").arg(
            InvestmentLedgerController::labelForRow(_row, trans->action()));
      }
        ...
        case 0:
          switch (trans->action()) {
            ...
            case InvestmentAction::Fee:
              return formatIdAccount(
                  trans->splitFor(InvestmentSplitType::CostProceeds));
            ...
            case InvestmentAction::Distribution:
              return formatIdAccount(
                  trans->splitFor(InvestmentSplitType::DistributionSource));

            case InvestmentAction::Transfer:
              if (trans->splitFor(InvestmentSplitType::InvestmentFrom)
                      .idAccount == account()->id()) {
                return formatIdAccount(
                    trans->splitFor(InvestmentSplitType::InvestmentTo));
              } else {
                return formatIdAccount(
                    trans->splitFor(InvestmentSplitType::InvestmentFrom));
              }
          ...
          switch (trans->action()) {
            ...
            case InvestmentAction::ShortSell:
              if (trans->hasSplitFor(InvestmentSplitType::Fee)) {
                return formatIdAccount(
                    trans->splitFor(InvestmentSplitType::Fee));
              } else {
                return QVariant();
              }
            ...
            case InvestmentAction::Distribution:
              return formatIdAccount(
                  trans->splitFor(InvestmentSplitType::DistributionDest));
          ...
          switch (trans->action()) {
            ...
            case InvestmentAction::Distribution:
              if (trans->hasSplitFor(InvestmentSplitType::Tax)) {
                return formatIdAccount(
                    trans->splitFor(InvestmentSplitType::Tax));
              } else {
                return QVariant();
              }
          ...
          switch (trans->action()) {
            ...
            case InvestmentAction::Sell:
              if (trans->hasSplitFor(InvestmentSplitType::GainLoss)) {
                return formatIdAccount(
                    trans->splitFor(InvestmentSplitType::GainLoss));
              } else {
                return QVariant();
              }
            ...
            case InvestmentAction::ReinvestDistrib:
              if (trans->hasSplitFor(InvestmentSplitType::CashInLieu)) {
                return formatIdAccount(
                    trans->splitFor(InvestmentSplitType::CashInLieu));
              } else {
                return QVariant();
              }
    ...
    case InvestmentLedgerColumn::QUANTITY: {
      ...
      switch (_row) {
        case 0:
          switch (trans->action()) {
            ...
            case InvestmentAction::Dividend:
            case InvestmentAction::Distribution:
              a = -trans->splitFor(InvestmentSplitType::DistributionSource)
                       .amount;
              inCurrency = true;
              break;
            ...
            case InvestmentAction::StockSplit:
              return tr("%1 for %2")
                  .arg(trans->splitFraction().first)
                  .arg(trans->splitFraction().second);
      ...
      return a == 0 ? QVariant()
                    : QVariant(inCurrency && !_editRole
                                   ? m_currency->formatAmount(a)
                                   : a.toString());
    }

    case InvestmentLedgerColumn::PRICE: {
      if (_row != 0) return QVariant();
...
QVariant InvestmentLedgerController::headerData(int _section,
...
bool InvestmentLedgerController::canEditTransaction(
    const Transaction* _transaction, QString* _message) const {
  if (!qobject_cast<const InvestmentTransaction*>(_transaction)) {
    if (_message)
      *_message = tr("This ledger can only edit investment transactions.");
    return false;
  } else {
    return true;
  }
...
QString InvestmentLedgerController::labelForRow(int _row,
...
void InvestmentLedgerController::editDistributionComposition(
    const QList<int>& _rows) {
  if (!_rows.count() ||
  ...
  for (int row : _rows) {
    if (row == newTransactionRow()) {
      ...
      InvestmentAction a =
          (InvestmentAction)m_buffer
              ->data(InvestmentLedgerColumn::ACTION, 0, true, this)
              .toInt();
      ...
      comp =
          qobject_cast<InvestmentLedgerBuffer*>(m_buffer)->distribComposition;
    } else {
      const InvestmentTransaction* tr =
...
void InvestmentLedgerController::openGainLossWizard(const QModelIndex& _index) {
  ...
  if (formGL.exec() == QDialog::Accepted) {
    ...
    QModelIndex idx =
        index(2, InvestmentLedgerColumn::QUANTITY, index(m_buffer->row, 0));
    emit dataChanged(idx, idx);
  }
}

void InvestmentLedgerController::addExtraActions(
    LedgerWidget* _widget, std::function<void(int, LedgerAction*)> _addAction) {
  LedgerAction* actGainLoss = new LedgerAction(
      new QAction(Core::icon("office-chart-line"), tr("Gain Loss &Wizard"),
                  this),
      true,
      [this, _widget](const QList<int>&) {
        return account()->isOpen() && hasModifiedRow() &&
  ...
  LedgerAction* actDistribComposition = new LedgerAction(
      new QAction(Core::icon("office-chart-pie"),
                  tr("D&istribution Composition"), this),
      true,
        ...
        if (account()->isOpen()) {
          if (hasModifiedRow()) {
            ...
            for (int row : rows) {
              InvestmentAction a =
                  (InvestmentAction)data(
                      index(row, InvestmentLedgerColumn::ACTION), Qt::EditRole)
                      .toInt();

              enableDist =
                  enableDist && (a == InvestmentAction::Distribution ||
                                 a == InvestmentAction::ReinvestDistrib);
            }
      ...
      [this, _widget]() {
        editDistributionComposition(_widget->selectedRows());
      });
...
void InvestmentLedgerBuffer::clear() {
  ...
  basisAdjustment = taxPaid = swapTo = cashInLieu = fee = quantity =
      pricePerShare = gainLoss = 0;
  idFeeAccount = idTaxAccount = idCashInLieuAccount = idDivDistToAccount =
      idGainLossAccount = Constants::NO_ID;
...
void InvestmentLedgerBuffer::displayNetAmount() {
  ...
  switch (action) {
    case InvestmentAction::Buy:
      net = pricePerShare * quantity + getNetAmount(idFeeAccount, fee);
      break;
  ...
  emit showMessage(tr("Net: %1").arg(
      CurrencyManager::instance()->get(cur)->formatAmount(net)));
}

QVariant InvestmentLedgerBuffer::data(
    int _column, int _row, bool _editRole,
    const LedgerController* _controller) const {
  const InvestmentLedgerController* inv_con =
      qobject_cast<const InvestmentLedgerController*>(_controller);

  auto formatToQVariant = [_editRole, inv_con](const Amount& _a,
                                               bool inCurrency) {
    return _editRole
  ...
  switch (_column) {
    case InvestmentLedgerColumn::ACTION:
      if (_editRole) {
        ...
        return QString("%1:").arg(
            InvestmentLedgerController::labelForRow(_row, action));
      }
    case InvestmentLedgerColumn::TRANSFER: {
      auto formatIdAccount = [_editRole, _controller](int idAccount,
                                                      const QString& currency) {
        if (_editRole) {
          return QVariant::fromValue<AccountCurrency>(
              AccountCurrency(idAccount, currency));
        } else {
          return idAccount == Constants::NO_ID
                     ? QVariant()
                     : QVariant(Account::getTopLevel()->getPath(
      ...
      switch (_row) {
        case 0:
          switch (action) {
            ...
            case InvestmentAction::StockSplit:
              return tr("%1 for %2")
                  .arg(splitFraction.first)
                  .arg(splitFraction.second);
...
bool InvestmentLedgerBuffer::setData(int _column, int _row,
                                     const QVariant& _value,
                                     LedgerController* _controller) {
  // This represents the "groups" for idTransfer: if we change in the same
  // group, we do not need to reset idTransfer (same account types), otherwise
  // it is necessary.
  ...
  switch (_column) {
    case InvestmentLedgerColumn::ACTION:
      if (_row == 0 && action != (InvestmentAction)_value.toInt()) {
        ...
        if (cat.value(newAction) !=
            cat.value(action))  // cat[invalid] will return 0.
        {
    ...
    case InvestmentLedgerColumn::QUANTITY: {
      int curPrec =
          CurrencyManager::instance()->get(m_security->currency())->precision();
      int curSwap = 2;
      ...
      try {
        ...
        if (a)
          curSwap =
              SecurityManager::instance()->get(a->idSecurity())->precision();
      } catch (...) {
      }
      ...
      switch (_row) {
        case 0:
          switch (action) {
            ...
            case InvestmentAction::ReinvestDistrib:
            case InvestmentAction::Fee:
              quantity = Amount::fromUserLocale(_value.toString(),
                                                m_security->precision());
              ok = true;
              break;
        ...
        case 1:
          switch (action) {
            ...
            case InvestmentAction::Swap:
              ...
              //                case InvestmentAction::Distribution:
              //                    a =
              //                    trans->splitFor(InvestmentSplitType::DistributionDest).amount;
              //                    break;
    ...
    case InvestmentLedgerColumn::PRICE:
      if (_row == 0) {
        switch (action) {
          ...
          case InvestmentAction::ReinvestDistrib:
          case InvestmentAction::Fee:
            pricePerShare = Amount::fromUserLocale(_value.toString(),
                                                   m_security->precision());
            ok = true;
            break;
...
void InvestmentLedgerBuffer::load(const Transaction* _transaction,
                                  const LedgerController* _controller) {
  const InvestmentTransaction* tr =
      qobject_cast<const InvestmentTransaction*>(_transaction);
  ...
  if (tr->hasSplitFor(InvestmentSplitType::CashInLieu)) {
    ...
    idCashInLieuAccount =
        tr->splitFor(InvestmentSplitType::CashInLieu).idAccount;
  }

  if (tr->hasSplitFor(InvestmentSplitType::DistributionDest)) {
    idDivDistToAccount =
        tr->splitFor(InvestmentSplitType::DistributionDest).idAccount;
  }
  ...
  if (tr->action() == InvestmentAction::Transfer ||
    ...
    if (tr->splitFor(InvestmentSplitType::InvestmentFrom).idAccount ==
  ...
  // Now load the normal stuff (no, date, attachments, etc.) Won't load splits
  // anymore since our specified version of loadSplits() is empty.
...
QStringList InvestmentLedgerBuffer::validate(
    int& _firstErrorColumn, const LedgerController* _controller) {
  QStringList errors;
  ...
  auto checkAccountCurrency = [&invCur](int _idAccount) {
    ...
    return a->supportsCurrency(invCur);
  };

  auto addError = [&_firstErrorColumn, &errors](const QString& _error,
                                                int _column) {
    errors << _error;
    if (_firstErrorColumn == -1) _firstErrorColumn = _column;
  };
  ...
  switch (action) {
    ...
    case InvestmentAction::ShortCover:
      ...
      if (idGainLossAccount != Constants::NO_ID || gainLoss != 0) {
        if (!checkAccount(idGainLossAccount)) {
          ...
          addError(tr("The gain/loss account's currency must be the same as "
                      "the security's currency (%1).")
                       .arg(invCur),
                   InvestmentLedgerColumn::TRANSFER);
        }
    ...
    case InvestmentAction::Fee:
      ...
      if (!checkAccount(idTransfer)) {
        ...
        addError(tr("The cost/proceeds account's currency must be the same as "
                    "the security's currency (%1).")
                     .arg(invCur),
                 InvestmentLedgerColumn::TRANSFER);
      }
      ...
      if (pricePerShare < 0) {
        addError(tr("The price per share must be at least zero.").arg(invCur),
                 InvestmentLedgerColumn::TRANSFER);
      }
      ...
      if (idFeeAccount != Constants::NO_ID || fee != 0) {
        if (!checkAccount(idFeeAccount)) {
          ...
          addError(tr("The fee account's currency must be the same as the "
                      "security's currency (%1).")
                       .arg(invCur),
                   InvestmentLedgerColumn::TRANSFER);
        }
      ...
      if (idTaxAccount != Constants::NO_ID || taxPaid != 0) {
        if (!checkAccount(idTaxAccount)) {
          ...
          addError(tr("The tax account's currency must be the same as the "
                      "security's currency (%1).")
                       .arg(invCur),
                   InvestmentLedgerColumn::TRANSFER);
        }
      ...
      if (action == InvestmentAction::ReinvestDistrib ||
        ...
        if (idCashInLieuAccount != Constants::NO_ID || cashInLieu != 0) {
          if (!checkAccount(idCashInLieuAccount)) {
            ...
            addError(tr("The cash in lieu account's currency must be the same "
                        "as the security's currency (%1).")
                         .arg(invCur),
                     InvestmentLedgerColumn::TRANSFER);
          }
    ...
    case InvestmentAction::Swap:
      ...
      if ((quantity > 0 && swapTo > 0) || (quantity < 0 && swapTo < 0)) {
    ...
    case InvestmentAction::StockSplit:
      if (splitFraction.first <= 0 || splitFraction.second <= 0) {
        addError(tr("The split fraction is invalid. Both parts of the fraction "
    ...
    case InvestmentAction::Distribution:
      if (quantity <= 0) {
        addError(
            tr("The dividend/distribution amount must be greater than zero."),
            InvestmentLedgerColumn::QUANTITY);
      }
      ...
      if (!checkAccount(idTransfer)) {
        ...
        addError(tr("The source account's currency must be the same as the "
                    "security's currency (%1).")
                     .arg(invCur),
                 InvestmentLedgerColumn::TRANSFER);
      }
      ...
      if (!checkAccount(idDivDistToAccount)) {
        ...
        addError(tr("The target account's currency must be the same as the "
                    "security's currency (%1).")
                     .arg(invCur),
                 InvestmentLedgerColumn::TRANSFER);
      }
      ...
      if (taxPaid > quantity) {
        addError(tr("The tax must be smaller than the dividend/distribution "
                    "amount."),
                 InvestmentLedgerColumn::QUANTITY);
      }
...
bool InvestmentLedgerBuffer::saveToTransaction(Transaction* _transaction,
                                               LedgerController* _controller) {
  InvestmentTransaction* inv_transaction =
      qobject_cast<InvestmentTransaction*>(_transaction);
  ...
  switch (action) {
    ...
    case InvestmentAction::ShortCover:
      ...
      // Compute net amount
      net = quantity * pricePerShare + fee + taxPaid;
      ...
      addTradingSplits();
      inv_transaction->makeBuySellFee(action, pricePerShare, splits, types,
                                      lots);
      break;
    ...
    case InvestmentAction::ReinvestDistrib:
      ...
      addTradingSplits();
      inv_transaction->makeReinvestedDivDist(action, pricePerShare, splits,
                                             types, distribComposition);
      break;
    ...
    case InvestmentAction::Dividend:
      ...
      splits << Transaction::Split(quantity - taxPaid, idDivDistToAccount,
                                   invCur);
      ...
      inv_transaction->makeDivDist(action, _controller->account()->id(), splits,
                                   types, distribComposition);
      break;
    ...
    case InvestmentAction::UndistributedCapitalGain:
      inv_transaction->makeUndistributedCapitalGain(
          _controller->account()->id(), basisAdjustment, taxPaid);
      break;

    case InvestmentAction::CostBasisAdjustment:
      inv_transaction->makeCostBasisAdjustment(_controller->account()->id(),
                                               basisAdjustment);
      break;
...
QWidget* InvestmentLedgerWidgetDelegate::createEditor(
    QWidget* _parent, const QStyleOptionViewItem& _option,
    const QModelIndex& _index) const {
  Security* cur_sec =
      static_cast<const InvestmentLedgerController*>(_index.model())
          ->security();
  const Account* cur_account =
  ...
  const InvestmentAction action =
      (InvestmentAction)_index.model()
  ...
  switch (_index.column()) {
    case InvestmentLedgerColumn::ACTION: {
      ...
      cboType->addItem(
          InvestmentTransaction::actionToString(InvestmentAction::Buy),
          (int)InvestmentAction::Buy);
      cboType->addItem(
          InvestmentTransaction::actionToString(InvestmentAction::Sell),
          (int)InvestmentAction::Sell);
      cboType->addItem(
          InvestmentTransaction::actionToString(InvestmentAction::ShortSell),
          (int)InvestmentAction::ShortSell);
      cboType->addItem(
          InvestmentTransaction::actionToString(InvestmentAction::ShortCover),
          (int)InvestmentAction::ShortCover);
      cboType->addItem(
          InvestmentTransaction::actionToString(InvestmentAction::Transfer),
          (int)InvestmentAction::Transfer);
      cboType->addItem(
          InvestmentTransaction::actionToString(InvestmentAction::Swap),
          (int)InvestmentAction::Swap);
      cboType->addItem(
          InvestmentTransaction::actionToString(InvestmentAction::StockSplit),
          (int)InvestmentAction::StockSplit);
      cboType->addItem(
          InvestmentTransaction::actionToString(InvestmentAction::Dividend),
          (int)InvestmentAction::Dividend);
      cboType->addItem(
          InvestmentTransaction::actionToString(InvestmentAction::Distribution),
          (int)InvestmentAction::Distribution);
      cboType->addItem(
          InvestmentTransaction::actionToString(InvestmentAction::ReinvestDiv),
          (int)InvestmentAction::ReinvestDiv);
      cboType->addItem(InvestmentTransaction::actionToString(
                           InvestmentAction::ReinvestDistrib),
      ...
      cboType->addItem(
          InvestmentTransaction::actionToString(InvestmentAction::Fee),
      ...
      switch (row) {
          ...
          switch (action) {
            ...
            case InvestmentAction::ShortSell:
              setCurrentEditor(new AccountSelector(
                  {.selectorFlags =
                   ...
                   .currencyRestrict = cur_sec->currency()},
                  _parent));
              break;
            ...
            case InvestmentAction::Fee:
              setCurrentEditor(new AccountSelector(
                  {.selectorFlags =
                       AccountSelectorFlags::Flag_MultipleCurrencies,
                   .typeFlags = AccountTypeFlags::Flag_IncomeExpense,
                   .currencyRestrict = cur_sec->currency()},
                  _parent));
              break;
            ...
            case InvestmentAction::Transfer:
              setCurrentEditor(new AccountSelector(
                  {.selectorFlags =
                       AccountSelectorFlags::Flag_MultipleCurrencies,
                   .idSecurityRestrict = cur_sec->id(),
                   .idExcludeAccount = cur_account->id()},
                  _parent));
              break;
        ...
        case 1:
          switch (action) {
            ...
            case InvestmentAction::Swap:
              setCurrentEditor(new AccountSelector(
                  {.typeFlags = AccountTypeFlags::Flag_Investment,
                   .idExcludeAccount = cur_account->id()},
                  _parent));
              break;
        ...
        default:
          setCurrentEditor(new AccountSelector(
              {.selectorFlags = AccountSelectorFlags::Flag_MultipleCurrencies},
              _parent));
          break;
    ...
    case InvestmentLedgerColumn::QUANTITY: {
      if (row == 0 && action == InvestmentAction::StockSplit) {
        ...
        int prec =
            CurrencyManager::instance()->get(cur_sec->currency())->precision();
...
void InvestmentLedgerWidgetDelegate::setEditorData(
  ...
  switch (_index.column()) {
    case InvestmentLedgerColumn::ACTION: {
      ...
      cboType->setCurrentIndex(cboType->findData(
          _index.model()->data(_index, Qt::EditRole).toInt()));
      if (cboType->currentIndex() == -1) cboType->setCurrentIndex(0);
      break;
    }
    case InvestmentLedgerColumn::QUANTITY:
      if (row == 0 && action == InvestmentAction::StockSplit) {
        SplitFractionWidget* w = static_cast<SplitFractionWidget*>(_editor);
        QStringList l = _index.model()
                            ->data(_index, Qt::EditRole)
                            .toString()
    ...
    case InvestmentLedgerColumn::PRICE: {
      ...
      amtEdit->setAmount(Amount::fromUserLocale(
...
void InvestmentLedgerWidgetDelegate::setModelData(
