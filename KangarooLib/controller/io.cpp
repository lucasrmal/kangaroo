...
namespace KLib {
        ...
        m_fileVersion(LATEST_FILE_VERSION)
    {
        ...
        registerFileManager(StdTags::PICTURE_MGR,  StdTags::PICTURE_MGR,  PictureManager::instance());
        registerFileManager(StdTags::DOCUMENT_MGR, StdTags::DOCUMENT_MGR, DocumentManager::instance());
        ...
        while(!xml.atEnd() && !xml.hasError())
        {
           ...
           else if (xml.name() == StdTags::ROOT)
           {
               if (token == QXmlStreamReader::StartElement)
               {
                   ...
                   {
                       throw IOException(tr("The version of this file unsupported by this version. "
...
//        delete file;
    ...
    QString IO::getOptAttribute(const QString& _name, QXmlStreamAttributes& _attrib, const QVariant& _default)
    {
        if (_attrib.hasAttribute(_name))
        {
            return _attrib.value(_name).toString();
    ...
    void IO::registerStored(const QString& _xmlKey, IStored* _st)
    {
        ...
        {
            throw IOException(tr("This XML key is invalid. XML keys can only be composed of letters and underscores."));
        }
        else if (m_registered.contains(_xmlKey))
        {
            throw IOException(tr("A Storeable with this key is already registered: %1.").arg(_xmlKey));
        }
    ...
    void IO::registerFileManager(const QString& _xmlKey, const QString& _directoryName, IFileManager* _manager)
    {
        QRegExp reg("^[A-Za-z_]+$");

        if (!reg.exactMatch(_directoryName))
        {
            throw IOException(tr("This directory is invalid. Directories name can only be composed of letters and underscores."));
        }
        else if (m_fileManagers.contains(_directoryName))
        {
            throw IOException(tr("A IFileManager with this directory is already registered: %1.").arg(_directoryName));
        }
        else if (m_registered.contains(_xmlKey))
        {
            throw IOException(tr("A Storeable with this key is already registered: %1.").arg(_xmlKey));
