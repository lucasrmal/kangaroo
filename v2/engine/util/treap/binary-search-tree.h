...
class BinarySearchTree : public BinaryTree<Node> {
 protected:
  ...
  int n;
  T null;
  virtual Node *findLast(T x) const;
 ...
 public:
  ...
  virtual T find(T x) const;
  virtual T findEQ(T x) const;
  virtual int size() const;
  virtual void clear();
  virtual Node *findEQNode(T x) const;
  virtual Node *findLTNode(T x) const;
  virtual Node *findLastNode() const;
  virtual Node *findFirstNode() const;
};
...
BinarySearchTree<Node, T>::BinarySearchTree() {
  this->null = T();  //(T)NULL;  // won't work for non-primitive types
  n = 0;
}
...
template <class Node, class T>
Node *BinarySearchTree<Node, T>::findLast(T x) const {
  Node *w = r, *prev = nil;
  while (w != nil) {
    prev = w;
...
template <class Node, class T>
Node *BinarySearchTree<Node, T>::findLastNode() const {
  Node *w = r, *prev = r;
  while (w != nil) {
    prev = w;
    w = w->right;
...
template <class Node, class T>
Node *BinarySearchTree<Node, T>::findFirstNode() const {
  Node *w = r, *prev = r;
  while (w != nil) {
    prev = w;
    w = w->left;
...
template <class Node, class T>
Node *BinarySearchTree<Node, T>::findLTNode(T x) const {
  Node *u = r, *z = nil;
  while (u != nil) {
...
template <class Node, class T>
Node *BinarySearchTree<Node, T>::findEQNode(T x) const {
  Node *u = r, *z = nil;
...
template <class Node, class T>
T BinarySearchTree<Node, T>::findEQ(T x) const {
  Node *w = r;
  while (w != nil) {
    int comp = compare(x, w->x);
    if (comp < 0) {
...
template <class Node, class T>
T BinarySearchTree<Node, T>::find(T x) const {
  Node *w = r, *z = nil;
  while (w != nil) {
    int comp = compare(x, w->x);
    if (comp < 0) {
...
template <class Node, class T>
inline int BinarySearchTree<Node, T>::size() const {
  return n;
}

template <class Node, class T>
inline void BinarySearchTree<Node, T>::clear() {
...
/*
template<class T>
BinarySearchTree1<T*>::BinarySearchTree1() : BinarySearchTree<BSTNode1<T*>,
T*>(NULL) {
}
*/
