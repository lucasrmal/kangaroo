...
namespace KLib
{
    ...
    void Currency::setName(const std::string& _name)
    {
        m_name = _name;
        if (!onHoldToModify())
            emit modified();
    ...
    void Currency::setCustomSymbol(const std::string& _symbol)
    {
        m_symbol = _symbol;
        if (!onHoldToModify())
            emit modified();
    ...
    void Currency::setCode(const std::string& _code)
    {
        if (_code.length() != 3)
        {
    ...
    void Currency::save(QXmlStreamWriter& _writer) const
    {
        ...
        _writer.writeAttribute("precision", std::string::number(m_precision));
    }
    ...
    std::string Currency::formatAmount(const Amount& _amount) const
    {
        return symbol() + _amount.toPrecision(precision()).toString();
    }

    Currency* CurrencyManager::add(const std::string &_code,
                                   const std::string& _name,
                                   const std::string& _customSymbol,
    ...
    bool CurrencyManager::has(const std::string& _code) const
    {
        return m_index.contains(_code);
    }

    Currency* CurrencyManager::get(const std::string& _code) const
    {
    ...
    Currency* CurrencyManager::get(const std::string& _code, bool _add)
    {
        if (m_index.contains(_code))
        {
            return at(m_index[_code]);
    ...
    void CurrencyManager::remove(const std::string& _code)
    {
        if (m_index.contains(_code))
        {
