...
AccountRewardsTab::AccountRewardsTab(QWidget *parent) :
    AccountEditTab(parent)
{
    ...
    m_cboTargetAccount = new AccountSelector(Flag_None,
                                             AccountTypeFlags::Flag_None,
                                             Constants::NO_ID, this);

    m_txtTargetAccountName = new QLineEdit(this);
    m_cboTargetParentAccount = new AccountSelector(Flag_Placeholders,
                                             AccountTypeFlags::Flag_Asset,
                                             Constants::NO_ID, this);
    ...
    m_cboSourceAccount = new AccountSelector(Flag_None,
                                             AccountTypeFlags::Flag_None,
                                             Constants::NO_ID, this);

    m_txtSourceAccountName = new QLineEdit(this);
    m_cboSourceParentAccount = new AccountSelector(Flag_Placeholders,
    ...
    connect(m_optNoProgram, &QRadioButton::toggled,
            this, &AccountRewardsTab::onNoProgram);
    connect(m_optExistingProgram, &QRadioButton::toggled,
            this, &AccountRewardsTab::onExistingProgram);
    connect(m_optNewProgram, &QRadioButton::toggled,
            this, &AccountRewardsTab::onNewProgram);

    connect(m_optExistingTargetAccount, &QRadioButton::toggled,
            this, &AccountRewardsTab::onExistingTargetAccount);
    connect(m_optNewTargetAccount, &QRadioButton::toggled,
            this, &AccountRewardsTab::onNewTargetAccount);

    connect(m_optExistingSourceAccount, &QRadioButton::toggled,
            this, &AccountRewardsTab::onExistingSourceAccount);
    connect(m_optNewSourceAccount, &QRadioButton::toggled,
            this, &AccountRewardsTab::onNewSourceAccount);

    connect(m_btnAddCurrency, &QPushButton::clicked,
            this, &AccountRewardsTab::addCurrency);

    connect(m_btnEditTiers, &QPushButton::clicked,
            this, &AccountRewardsTab::editTiers);

    connect(m_cboRewardsProgram, SIGNAL(currentIndexChanged(int)),
            this, SLOT(onProgramOrCurrencyChanged()));
    connect(m_cboCurrency, SIGNAL(currentIndexChanged(int)),
            this, SLOT(onProgramOrCurrencyChanged()));
...
void AccountRewardsTab::onProgramOrCurrencyChanged()
{
    ...
    if (m_optExistingProgram->isChecked())
    {
        ...
        {
            m_cboTargetAccount->setShowOnlyCurrency(nullptr, AccountTypeFlags::Flag_None);
            m_cboSourceAccount->setShowOnlyCurrency(nullptr, AccountTypeFlags::Flag_None);
        ...
        {
            currency = RewardsProgramManager::instance()->get(m_cboRewardsProgram->currentData().toInt())->currency();
        }
    }
    else //New
    {
    ...
    {
        m_cboTargetAccount->setShowOnlyCurrency(CurrencyManager::instance()->get(currency), AccountTypeFlags::Flag_Asset);
        m_cboTargetAccount->setCurrentText("");

        m_cboSourceAccount->setShowOnlyCurrency(CurrencyManager::instance()->get(currency), AccountTypeFlags::Flag_Income);
        m_cboSourceAccount->setCurrentText("");
...
QString AccountRewardsTab::tabName() const
{
    return tr("Rewards");
}

QString AccountRewardsTab::tabIcon() const
{
    return "favorites";
}
...
void AccountRewardsTab::fillData(const KLib::Account* _account)
{
    ...
    if (_account->properties()->contains(RewardsProgram::PROP_REWARDS_PROGRAM))
    {
        try
        {
            RewardsProgram* p = RewardsProgramManager::instance()->get(_account->properties()->get(RewardsProgram::PROP_REWARDS_PROGRAM).toInt());

            m_optExistingProgram->setChecked(true);
            m_cboRewardsProgram->setCurrentIndex(m_cboRewardsProgram->findData(p->id()));
            onProgramOrCurrencyChanged();

            try
            {
                Account* target = Account::getTopLevel()
                                  ->account(_account->properties()
                                                    ->get(RewardsProgram::PROP_REWARDS_TARGET_ACCOUNT).toInt());
                Account* source = Account::getTopLevel()
                                  ->account(_account->properties()
                                                    ->get(RewardsProgram::PROP_REWARDS_SOURCE_ACCOUNT).toInt());

                m_optExistingTargetAccount->setChecked(true);
                m_optExistingSourceAccount->setChecked(true);

                //Set the account, but do this a little later to let AccountSelector the time to load the accounts...
                m_timer->setInterval(500);
...
void AccountRewardsTab::save(KLib::Account* _account) const
{
    ...
    if (set)
    {
        ...
        if (m_optExistingProgram->isChecked())
        {
            p = RewardsProgramManager::instance()->get(m_cboRewardsProgram->currentData().toInt());
        }
        else //New program
        {
            ...
            p = RewardsProgramManager::instance()
                                ->add(m_txtProgramName->text(),
        ...
        {
            target = m_cboTargetParentAccount->currentAccount()
                            ->addChild(m_txtTargetAccountName->text(),
                                       RewardsProgram::REWARDS_ACCOUNT_TYPE,
                                       p->currency(),
                                       Constants::NO_ID);
        }
        ...
        {
            source = m_cboSourceParentAccount->currentAccount()
                            ->addChild(m_txtSourceAccountName->text(),
                                       AccountType::INCOME,
                                       p->currency(),
                                       Constants::NO_ID);
        }

        _account->properties()->set(RewardsProgram::PROP_REWARDS_PROGRAM,
                                    p->id());
...
QStringList AccountRewardsTab::validate() const
{
    ...
    {
        if (m_optExistingProgram->isChecked() && m_cboRewardsProgram->currentIndex() < 0)
        {
        ...
        if (m_optExistingTargetAccount->isChecked() && !m_cboTargetAccount->currentAccount())
        {
            errors << tr("Select a deposit account for the rewards program.");
        }
        if (m_optNewTargetAccount->isChecked() && m_txtTargetAccountName->text().isEmpty())
        {
            errors << tr("Enter a name for the deposit rewards account.");
        }
        if (m_optNewTargetAccount->isChecked() && !m_cboTargetParentAccount->currentAccount())
        {
            errors << tr("Select a parent for the deposit rewards account.");
        }

        if (m_optExistingSourceAccount->isChecked() && !m_cboSourceAccount->currentAccount())
        {
            errors << tr("Select an income account for the rewards program.");
        }
        if (m_optNewSourceAccount->isChecked() && m_txtSourceAccountName->text().isEmpty())
        {
            errors << tr("Enter a name for the income rewards account.");
        }
        if (m_optNewSourceAccount->isChecked() && !m_cboSourceParentAccount->currentAccount())
        {
