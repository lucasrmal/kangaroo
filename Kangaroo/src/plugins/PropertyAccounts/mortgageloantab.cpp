...
MortgageLoanTab::MortgageLoanTab(QWidget* _parent) :
    AccountEditTab(_parent)
{
    ...
    m_cboAccount = new AccountSelector(filterFn, this);
...
void MortgageLoanTab::fillData(const KLib::Account* _account)
{
    ...
    {
        m_cboAccount->setCurrentAccount(_account->properties()->get(RealEstateHelpers::LINKED_PROPERTY, -1).toInt());
    }
}
...
QStringList MortgageLoanTab::validate() const
{
    return {};
}
