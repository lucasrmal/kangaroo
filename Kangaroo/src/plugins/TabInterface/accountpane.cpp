...
QAction* AccountPane::m_actReassign = nullptr;
QAction* AccountPane::m_actShowClosed = nullptr;

const QColor AccountPaneTree::CATEGORY_COLOR = QColor(230, 230, 230);

AccountPane::AccountPane(QWidget* parent) : QWidget(parent) {
  // Create the menu
  if (!m_accountMenu) {
    m_accountMenu = ActionManager::instance()->insertMenu(
        STD_MENUS::MENUBAR, STD_MENUS::MENU_MANAGE, "Account", tr("&Account"));

    m_actCreate = m_accountMenu->menu()->addAction(
        Core::icon("create-bank-account"), tr("Add &Account"), this,
        SLOT(createAccount()));
    m_actEdit = m_accountMenu->menu()->addAction(
        Core::icon("edit"), tr("&Edit Account"), this, SLOT(editAccount()));
    m_accountMenu->menu()->addSeparator();
    m_actClose = m_accountMenu->menu()->addAction(
        Core::icon("close-bank-account"), tr("&Close Account"), this,
        SLOT(closeAccount()));
    m_actReopen = m_accountMenu->menu()->addAction(
        Core::icon("reopen-bank-account"), tr("Re-&open Account"), this,
        SLOT(reopenAccount()));
    m_actReassign = m_accountMenu->menu()->addAction(
        Core::icon("reassign"), tr("&Reassign All Transactions"), this,
        SLOT(reassignAllTransactions()));
    m_actRemove = m_accountMenu->menu()->addAction(Core::icon("trash"),
                                                   tr("&Delete Account"), this,
                                                   SLOT(removeAccount()));
    m_accountMenu->menu()->addSeparator();
    m_actShowClosed =
        m_accountMenu->menu()->addAction(tr("&Show Closed Accounts"));
    m_actShowClosed->setCheckable(true);
    m_actShowClosed->setChecked(false);
    connect(m_actShowClosed, &QAction::triggered, this,
            &AccountPane::showClosedAccounts);

    ActionManager::instance()->registerAction(m_actCreate, "Account.Add",
                                              QKeySequence("F6"));
    ...
    ActionManager::instance()->registerAction(
        m_actReassign, "Account.ReassignAllTransactions");
    ActionManager::instance()->registerAction(m_actRemove, "Account.Remove");
    ActionManager::instance()->registerAction(m_actShowClosed,
                                              "Account.ShowAll");
    ...
    connect(m_actCreate, &QAction::triggered, this,
            &AccountPane::createAccount);
    ...
    connect(m_actReopen, &QAction::triggered, this,
            &AccountPane::reopenAccount);
    connect(m_actReassign, &QAction::triggered, this,
            &AccountPane::reassignAllTransactions);
    connect(m_actRemove, &QAction::triggered, this,
            &AccountPane::removeAccount);
    connect(m_actShowClosed, &QAction::triggered, this,
            &AccountPane::showClosedAccounts);
  }
  ...
  //    connect(m_treeAccounts, &QTreeView::activated, this,
  //    &AccountPane::onItemSelected);
  connect(m_treeAccounts, &QTreeView::clicked, this,
          &AccountPane::onItemSelected);

  connect(LedgerTabManager::instance(),
          &LedgerTabManager::currentAccountChanged, this,
          &AccountPane::onAccountTabChanged);
...
void AccountPane::reassignAllTransactions() {
...
void AccountPane::onAccountTabChanged(Account* _account) {
  if (_account) {
    ...
    m_actReassign->setEnabled(_account->type() == AccountType::INCOME ||
                              _account->type() == AccountType::EXPENSE);
...
QSize AccountPaneDelegate::sizeHint(const QStyleOptionViewItem& _option,
                                    const QModelIndex& _index) const {
  QSize s = QStyledItemDelegate::sizeHint(_option, _index);

  if (_index.isValid()) {
    SimpleAccountController::AccountNode* n =
        static_cast<SimpleAccountController::AccountNode*>(
            _index.internalPointer());
    const SimpleAccountController* controller =
        static_cast<const SimpleAccountController*>(_index.model());
...
void AccountPaneDelegate::paint(QPainter* _painter,
                                const QStyleOptionViewItem& _option,
                                const QModelIndex& _index) const {
  // First, compute how much size the amount takes.
  QModelIndex amountIdx =
      _index.model()->index(_index.row(), _index.column() + 1, _index.parent());
...
AccountPaneTree::AccountPaneTree(KLib::SimpleAccountController* _model,
                                 QWidget* parent)
    : QTreeView(parent), m_model(_model), m_rightClickedAccount(nullptr) {
  setModel(_model);
  setMouseTracking(true);
  setRootIsDecorated(false);

  m_contextMenu_generic = new QMenu(tr("Accounts"), this);
  m_contextMenu_generic->addAction(
      ActionManager::instance()->command("Account.Add")->action());
  m_contextMenu_generic->addAction(
      ActionManager::instance()->command("Account.ShowAll")->action());

  m_contextMenu_account = new QMenu(tr("Account"), this);
  m_actCreate = m_contextMenu_account->addAction(
      Core::icon("create-bank-account"), tr("Add &Account"), this,
      SLOT(createAccount()));
  m_contextMenu_account->addSeparator();
  m_actEdit = m_contextMenu_account->addAction(
      Core::icon("edit"), tr("&Edit Account"), this, SLOT(editAccount()));
  m_actClose = m_contextMenu_account->addAction(
      Core::icon("close-bank-account"), tr("&Close Account"), this,
      SLOT(closeAccount()));
  m_actRemove = m_contextMenu_account->addAction(
      Core::icon("trash"), tr("&Delete Account"), this, SLOT(removeAccount()));

  connect(LedgerTabManager::instance(),
          &LedgerTabManager::currentAccountChanged, this,
          &AccountPaneTree::onAccountTabChanged);
}

QHash<int, bool> AccountPaneTree::expandedState() const {
  QHash<int, bool> state;

  for (int i = (int)AccountClassification::CASH_FLOW;
       i < (int)AccountClassification::NumCategories; ++i) {
...
void AccountPaneTree::drawBranches(QPainter*, const QRect&,
                                   const QModelIndex&) const {}

void AccountPaneTree::drawRow(QPainter* _painter,
                              const QStyleOptionViewItem& _option,
                              const QModelIndex& _index) const {
  ...
  if (_index.isValid()) {
    ...
    if (n && n->category != AccountClassification::INVALID) {
      ...
      QStyleOptionViewItem option2 = _option;
      QRect boxRect(_option.rect.left() + (indentation() * 2),
                    _option.rect.top() + 1,
                    _option.rect.width() - (indentation() * 2 + 4),
                    _option.rect.height() - 2);
      ...
      if (m_model->indexIsAfterTypeChange(_index)) {
        ...
        _painter->drawLine(
            _option.rect.left() + MARGIN_LINE + (indentation() * 2),
            _option.rect.top(), _option.rect.right() - MARGIN_LINE,
            _option.rect.top());
        _painter->restore();
...
void AccountPaneTree::mousePressEvent(QMouseEvent* _event) {
  ...
  if (i.isValid()) {
    SimpleAccountController::AccountNode* n =
        static_cast<SimpleAccountController::AccountNode*>(i.internalPointer());
