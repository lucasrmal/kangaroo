...
{
    m_chart = new ReturnChart(ReturnChartStyle::Line, ValueType::Amount, Qt::EditRole, this);
    m_chart->setModel(m_model);
    m_chart->setLegendVisible(false);

    m_selector = new AccountSelector(Flag_Placeholders,
                                     AccountTypeFlags::Flag_AllAssets | AccountTypeFlags::Flag_Liability
    ...
    connect(m_btnRefresh,     &QPushButton::clicked,            this, &AccountBalancesOverTime::refresh);
...
void AccountBalancesOverTime::refresh()
{
    ...
    try
    {
        currency = m_chkIncludeSubtree->isChecked() ? Account::getTopLevel()->mainCurrency()
                                                            : account->mainCurrency();

        if (currency.isEmpty()) //Security
        {
            currency = SecurityManager::instance()->get(account->idSecurity())->currency();
        }

        m_chart->setYAxisIndicator(CurrencyManager::instance()->get(currency)->symbol(), true);
    }
    catch (...) {}
    ...
    auto getAmount = [account, currency, this](const QDate& _date) {
        ...
        try
        {
            ...
            {
                return PriceManager::instance()->rate(account->idSecurity(), currency, _date)
