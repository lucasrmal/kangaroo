...
SpendingIncomeOverTime::SpendingIncomeOverTime(QWidget* _parent)
  ...
  m_chart = new ReturnChart(ReturnChartStyle::VerticalBar, ValueType::Amount,
                            Qt::EditRole, this);
  ...
  connect(m_btnRefresh, &QPushButton::clicked, this,
          &SpendingIncomeOverTime::refresh);
...
void SpendingIncomeOverTime::refresh() {
  ...
  m_chart->setYAxisIndicator(
      CurrencyManager::instance()
          ->get(m_selector->currentAccount()->mainCurrency())
          ->symbol(),
      true);
  ...
  QString currency = m_chkIncludeSubtree->isChecked()
