#include "spendingincomeovertime.h"
...
SpendingIncomeOverTime::SpendingIncomeOverTime(QWidget* _parent) :
    IChart(_parent),
    m_model(new QStandardItemModel(this))
{
    m_chart = new ReturnChart(ReturnChartStyle::VerticalBar, ValueType::Amount, Qt::EditRole, this);
    m_chart->setModel(m_model);
    m_chart->setLegendVisible(false);

    m_selector = new AccountSelector(Flag_Placeholders, AccountTypeFlags::Flag_IncomeExpense, Constants::NO_ID, this);
    ...
    m_btnRefresh = new QPushButton(Core::icon("view-refresh"), tr("Refresh"), this);
    ...
    m_cboDisplayType->addItem(tr("YTD 2 Year Comp"),  (int) PeriodType::YTDTwoYearsComp);
    m_cboDisplayType->addItem(tr("YTD 3 Year Comp"),  (int) PeriodType::YTDThreeYearsComp);
    ...
    connect(m_btnRefresh,     &QPushButton::clicked,            this, &SpendingIncomeOverTime::refresh);
    connect(m_cboDisplayType, SIGNAL(currentIndexChanged(int)), this, SLOT(updatePeriodSelection()));
    connect(m_selector,       SIGNAL(currentAccountChanged(KLib::Account*)), this, SLOT(updatePeriodSelection()));

    updatePeriodSelection();
}

void SpendingIncomeOverTime::updatePeriodSelection()
{
...
void SpendingIncomeOverTime::refresh()
{
    if (!m_selector->currentAccount())
        return;

    m_chart->setYAxisIndicator(CurrencyManager::instance()->get(m_selector->currentAccount()->mainCurrency())->symbol(), true);
    ...
    case PeriodType::Month:
        ...
        title = tr("%3 - %1 %2").arg(QDate::longMonthName(beginDate.month())).arg(beginDate.year());
        break;
    ...
    case PeriodType::YTDTwoYearsComp:
        ...
        title = tr("%3 - YTD %1-%2").arg(beginDate.year()).arg(QDate::currentDate().year());
        numCols = 2;
        break;
    ...
    m_model->setColumnCount(numCols); //Do not want to display stuff while we update
    m_model->setRowCount(numDataPoints);

    Account* account = m_selector->currentAccount();

    QString currency = m_chkIncludeSubtree->isChecked() ? Account::getTopLevel()->mainCurrency()
                                                        : account->mainCurrency();

    m_chart->setTitle(title.arg(account->name()) + QString(" (%1)").arg(currency));
    m_chart->setLegendVisible(numCols > 1);

    if (type == PeriodType::TwoYearsComp || type == PeriodType::ThreeYearsComp
        || type == PeriodType::YTDTwoYearsComp || type == PeriodType::YTDThreeYearsComp)
    {
    ...
    auto getAmount = [account, this](const QDate& _from, const QDate& _to) {

        if (m_chkIncludeSubtree->isChecked())
        {
            return account->treeValueBetween(_from, _to).toDouble();
        }
        else
        {
            return account->balanceBetween(_from, _to).toDouble();
        }
    };
    ...
    for (int i = 0; i < numDataPoints; ++i)
    {
        ...
        QDate endDate = beginDate.addDays(interval[0]).addMonths(interval[1]).addYears(interval[2]);
        ...
        case PeriodType::ThreeYearsComp:
            for (int j = 0; j < numCols; ++j)
            {
                m_model->setData(m_model->index(i, j),
                                 getAmount(beginDate.addYears(j), endDate.addYears(j).addDays(-1)),
                                 Qt::EditRole);
            }
            break;

        default:
            m_model->setData(m_model->index(i, 0),
                             getAmount(beginDate, endDate.addDays(-1)),
                             Qt::EditRole);
        }
