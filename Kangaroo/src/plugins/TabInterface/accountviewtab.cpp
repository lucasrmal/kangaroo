...
AccountViewTab::AccountViewTab(KLib::Account* _topLevel, bool _showOpenOnly, int _flags, QWidget *parent) :
  QWidget(parent),
  m_toolbar(nullptr),
  m_findBar(nullptr),
  m_currentAccount(nullptr)
{
  ...
  connect(m_accountTree, &AccountTreeWidget::currentAccountChanged,
          this, &AccountViewTab::onCurrentAccountChanged);
  ...
  m_actOpen = m_accountMenu->addAction(Core::icon("open-ledger"),
                                       tr("Open &Ledger"), this, SLOT(openLedger()));
  m_accountMenu->addSeparator();
  m_actCreate = m_accountMenu->addAction(Core::icon("create-bank-account"),
                                         tr("&New"), this, SLOT(createAccount()));
  m_actEdit = m_accountMenu->addAction(Core::icon("edit"),
                                       tr("&Edit"), this, SLOT(editAccount()));
  m_accountMenu->addSeparator();
  m_actCloseOpen = m_accountMenu->addAction(Core::icon("close-bank-account"),
                                            tr("&Close"), this, SLOT(closeOpenAccount()));

  m_actDelete = m_accountMenu->addAction(Core::icon("trash"),
                                         tr("&Delete"), this, SLOT(deleteAccount()));

  m_accountMenu->addSeparator();
  m_actShowAll = m_accountMenu->addAction(tr("&Show All Accounts"));
  m_actShowAll->setCheckable(true);
  m_actShowAll->setChecked(false);
  connect(m_actShowAll, &QAction::triggered, this, &AccountViewTab::showAllAccounts);
...
void AccountViewTab::resetAccountTree()
{
  m_accountTree->reset();
}
...
void AccountViewTab::findAccounts()
{
  if (!m_findBar)
  {
    ...
    QCheckBox* chkShowChildren = new QCheckBox(tr("Include children"), m_findBar);
    ...
    connect(actClose, &QAction::triggered, this, &AccountViewTab::doneFindAccounts);
    connect(txtSearch, &QLineEdit::textEdited, this, &AccountViewTab::filterAccounts);
    connect(chkShowChildren, &QCheckBox::toggled, this, &AccountViewTab::setIncludeChildrenInFilter);
