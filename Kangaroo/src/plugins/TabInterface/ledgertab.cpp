...
{    
    ...
    mnuNew->addAction(m_ledgerWidget->basicAction(BasicLedgerAction::NewTransaction)->action);
    mnuNew->addAction(m_ledgerWidget->basicAction(BasicLedgerAction::NewSchedule)->action);
    ...
    bottomToolbar->addAction(m_ledgerWidget->basicAction(BasicLedgerAction::Edit)->action);
    bottomToolbar->addAction(m_ledgerWidget->basicAction(BasicLedgerAction::Delete)->action);
    bottomToolbar->addAction(m_ledgerWidget->basicAction(BasicLedgerAction::Duplicate)->action);
    ...
    connect(m_ledgerWidget, &LedgerWidget::showMessage,      this, &LedgerTab::onMessageReceived);
    connect(m_ledgerWidget, &LedgerWidget::selectedRowsTotalChanged, this, &LedgerTab::updateBalance);
...
void LedgerTab::updateBalance(const Amount& _amount)
{
    ...
    {
        emit showMessage(tr("Selected total: %1")
...
{
    //We want to reemit the message ourselves so that LedgerTabManager can identify the sender.
...
//    m_icnEnterSchedule = Core::icon("enter-schedule");
    ...
    Core::instance()->mainWindow()->statusBar()->addPermanentWidget(m_lblStatusMessage);
    ...
    connect(Account::getTopLevel(), &Account::accountModified,
            this, &LedgerTabManager::onAccountModified);

    connect(Account::getTopLevel(), &Account::accountRemoved,
            this, &LedgerTabManager::onAccountRemoved);

    connect(LedgerManager::instance(), &LedgerManager::balanceTodayChanged,
            this, &LedgerTabManager::onBalanceTodayChanged);
}
...
void LedgerTabManager::onLoad()
{
    ...
    onCurrentTabChanged();
    connect(TabInterface::instance(), SIGNAL(currentChanged(int)), this, SLOT(onCurrentTabChanged()));
}
...
{
    LedgerTab* tab = static_cast<LedgerTab*>(TabInterface::instance()->currentWidget());
    ...
    {
        if (m_currentTab) //Only emit the changed signal if we go from an account to no account
        {
            m_currentTab = nullptr;
...
void LedgerTabManager::onAccountModified(KLib::Account* _a)
{
    if (m_tabIndex.contains(_a->id()))
    {
        ...
        if (tabIndex != -1)
        {
            ...
            TabInterface::instance()->setTabIcon(tabIndex, QIcon(_a->defaultPicture(true)));
        }
    }

}

void LedgerTabManager::onBalanceTodayChanged(int _idAccount, const Balances& _balances)
{
    Q_UNUSED(_balances)
...
void LedgerTabManager::openLedger(Account* _a)
{
    ...
    else
    {
        ...
        TabInterface::instance()->addRegisteredTab(w,
                                                   _a->name(),
                                                   TABNAME(_a),
                                                   true,
...
{
    //Simply remove the tab visually; if the user requests it later it will be already in memory and we'll show it.
    if (TabInterface::instance()->hasTab(TABNAME(_a)))
    {
...
void LedgerTabManager::registerLedger(LedgerTab* _tab)
{
    ...
    connect(_tab, &LedgerTab::showMessage,      this, &LedgerTabManager::setStatusBarMessage);
}
