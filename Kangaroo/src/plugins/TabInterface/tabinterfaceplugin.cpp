...
bool TabInterfacePlugin::initialize(QString& p_errorMessage)
{
    ...
    //Create menus
    ActionContainer* mnuView = ActionManager::instance()->actionContainer(STD_MENUS::MENU_VIEW);

    QAction* mnuDocks = ActionManager::instance()->actionContainer(STD_MENUS::MENU_VIEW_DOCKS)->menu()->menuAction();

    mnuShowAllAccounts = new QAction(Core::icon("bank"), tr("All Accounts"), mnuView->menu());
    mnuShowIncomeExpenses = new QAction(Core::icon("income-account"), tr("Income and Expense Accounts"), mnuView->menu());
    ...
    QAction* clt = mnuView->menu()->addAction(Core::icon("tab-close"),
                                               tr("Close current tab"),
                                               TabInterface::instance(),
                                               SLOT(closeCurrentTab()));

    QAction* clat = mnuView->menu()->addAction(Core::icon(""),
                                               tr("Close all tabs"),
                                               TabInterface::instance(),
                                               SLOT(closeAllCloseableTabs()));

    ActionManager::instance()->registerAction(clt, "View.CloseTab", QKeySequence::Close);
    ActionManager::instance()->registerAction(clat, "View.CloseAllTab", QKeySequence("Ctrl+Shift+W"));
    ActionManager::instance()->registerAction(mnuShowIncomeExpenses, "View.IncomeExpenseAccounts", QKeySequence("F10"));
    ActionManager::instance()->registerAction(mnuShowAllAccounts, "View.AllAccounts", QKeySequence("F9"));

    mnuShowIncomeExpenses->setEnabled(false);
    mnuShowAllAccounts->setEnabled(false);

    connect(mnuShowAllAccounts, &QAction::triggered, this, &TabInterfacePlugin::showAllAccountsTab);
    connect(mnuShowIncomeExpenses, &QAction::triggered, this, &TabInterfacePlugin::showIncomeExpenseTab);
...
void TabInterfacePlugin::showAllAccountsTab()
{
    ...
    {
        TabInterface::instance()->addRegisteredTab(m_allAccountsTab,
                                                    tr("All Accounts"), "allaccounts", true,
                                                   Core::icon("bank"));
    }
}
...
void TabInterfacePlugin::showIncomeExpenseTab()
{
    ...
    {
        TabInterface::instance()->addRegisteredTab(m_incomeExpenseTab,
                                                   tr("Income and Expense"), "incomeexpense", true,
...
QString TabInterfacePlugin::name() const
{
    return "TabInterface";
}

QString TabInterfacePlugin::version() const
{
    return "1.0";
}
...
QString TabInterfacePlugin::author() const
{
    return Core::APP_AUTHOR;
}

QString TabInterfacePlugin::copyright() const
{
    return Core::APP_COPYRIGHT;
}

QString TabInterfacePlugin::url() const
{
