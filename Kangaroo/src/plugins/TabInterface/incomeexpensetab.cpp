...
IncomeExpenseTab::IncomeExpenseTab(QWidget* _parent)
    : AccountViewTab(Account::getTopLevel(), true,
                     AccountTypeFlags::Flag_IncomeExpense, _parent) {
  // Date selector
  ...
  connect(m_cboBalanceSelector, SIGNAL(currentIndexChanged(int)), this,
          SLOT(onChangeBalanceType(int)));

  connect(m_spinYear, SIGNAL(valueChanged(int)), SLOT(updateBalanceDates()));
  connect(m_cboMonth, SIGNAL(currentIndexChanged(int)),
          SLOT(updateBalanceDates()));
  ...
  //    bottomToolbar->addAction(tr("Convert Dist"), this,
  //    SLOT(convertDividend()));
...
//        CAMSEGDialog* d = new CAMSEGDialog(selector,
//        CAMSEGDialog::DialogWithoutPicture, CAMSEGDialog::OkCancelButtons,
//        this); d->setBothTitles(tr("Convert Distribution?"));
...
//                if (!qobject_cast<InvestmentTransaction*>(t)) //If not already
...
//                        }
//                        else if
//                        (Account::generalType(Account::getTopLevel()->account(s.idAccount)->type())
//                        == AccountType::ASSET)
//                        {
...
//                        }
//                        else if
//                        (Account::getTopLevel()->account(s.idAccount)->type()
//                        == AccountType::EXPENSE)
//                        {
...
//                        InvestmentTransaction* itr = new
//                        InvestmentTransaction; itr->setDate(t->date());
...
void IncomeExpenseTab::updateBalanceDates() {
  ...
  switch ((BalanceType)m_cboBalanceSelector->currentIndex()) {
    ...
    case BalanceType::Month:
      ...
      end = QDate(m_spinYear->value(), m_cboMonth->currentIndex() + 1,
                  begin.daysInMonth());
      break;
