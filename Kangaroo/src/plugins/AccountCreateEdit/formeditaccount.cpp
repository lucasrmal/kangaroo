...
QList<FormEditAccount::fn_buildTab> FormEditAccount::m_registeredTabs =
    QList<FormEditAccount::fn_buildTab>();
...
FormEditAccount::FormEditAccount(KLib::Account* _account, QWidget* _parent)
    : CAMSEGDialog(new QTabWidget(), CAMSEGDialog::DialogWithoutPicture,
                   CAMSEGDialog::OkCancelButtons, _parent),
      m_account(_account) {
  if (_account) {
  ...
  QPushButton* btnSelectPicture =
      new QPushButton(Core::icon("picture"), tr("Pictu&re"), this);
  ...
  static_cast<QLabel*>(layout->labelForField(secLayout))
      ->setBuddy(m_cboSecurity);
  static_cast<QLabel*>(layout->labelForField(instLayout))
      ->setBuddy(m_cboInstitution);
  ...
  connect(m_cboParent, SIGNAL(currentAccountChanged(KLib::Account*)), this,
          SLOT(onParentAccountChanged(KLib::Account*)));
  connect(m_cboType, SIGNAL(currentIndexChanged(int)), this,
          SLOT(onAccountTypeChanged()));

  connect(m_btnAddSecurity, &QPushButton::clicked, this,
          &FormEditAccount::addSecurity);
  connect(m_btnAddInstitution, &QPushButton::clicked, this,
          &FormEditAccount::addInstitution);
  connect(btnSelectPicture, &QPushButton::clicked, m_pictureSelector,
          &PictureSelector::selectPicture);

  connect(m_txtName, SIGNAL(textChanged(QString)), this,
  ...
  if (m_account) {
    ...
    m_cboCurrency->setCurrentIndex(
        m_cboCurrency->findData(Account::getTopLevel()->mainCurrency()));
    updateTitle("");
  }
...
QSize FormEditAccount::sizeHint() const { return QSize(700, 500); }
...
void FormEditAccount::fillData() {
  ...
  m_cboCurrency->setCurrentIndex(
      m_cboCurrency->findData(m_account->mainCurrency()));
  m_cboSecurity->setCurrentIndex(
      m_cboSecurity->findData(m_account->idSecurity()));
  m_cboInstitution->setCurrentIndex(
      m_cboInstitution->findData(m_account->idInstitution()));
  m_pictureSelector->setIdPicture(m_account->idPicture());
  m_pictureSelector->setDefaultPicture(
      Core::pixmap(Account::typeToIcon(m_account->type())));

  m_cboParent->setCurrentIndex(
      m_cboParent->findData(m_account->parent()->id()));
  onParentAccountChanged(m_cboParent->currentAccount());
...
void FormEditAccount::onParentAccountChanged(KLib::Account* _account) {
  ...
  for (int t : types) {
    m_cboType->addItem(Core::icon(Account::typeToIcon(t)),
                       Account::typeToString(t), t);
  }
...
void FormEditAccount::onAccountTypeChanged() {
  ...
  if (m_account && !m_account->isPlaceholder() &&
      m_account->ledger()->count()) {
    m_cboSecurity->setEnabled(false);
    // m_cboCurrency->setEnabled(false);
  }
  ...
  if (m_cboType->currentIndex() != -1) {
    m_pictureSelector->setDefaultPicture(
        Core::pixmap(Account::typeToIcon(m_cboType->currentData().toInt())));
  } else {
  ...
  for (int i = 1; i < m_tabs->count() - 1;
       ++i)  // Exclude 1st (Main) and last (Note)
  {
...
void FormEditAccount::addSecurity() {
  /*
   * We need a timer since m_cboSecurity uses a proxy model to sort the
   * SecurityController. Because of that, the sort is done after setting the new
   * index, which results in problems. The timer bypasses that, setting the
   * index after the sort is done.
   */

  FormEditSecurity* form = new FormEditSecurity(
      FormEditSecurity::SecurityClass::Security, nullptr, this);
  int idSec = 0;
...
void FormEditAccount::addInstitution() {
  /*
   * We need a timer since m_cboInstitution uses a proxy model to sort the
   * InstitutionController. Because of that, the sort is done after setting the
   * new index, which results in problems. The timer bypasses that, setting the
   * index after the sort is done.
   */

  FormEditInstitutionPayee* form = new FormEditInstitutionPayee(
      FormEditInstitutionPayee::Edit_Institution, this);
  int idInst = 0;
  if (form->exec() == QDialog::Accepted) {
    idInst = form->addedId();
    m_timer->setInterval(500);
...
void FormEditAccount::accept() {
  ...
  if (m_cboType->currentData().toInt() != AccountType::INVESTMENT &&
      m_cboCurrency->currentIndex() == -1) {
    errors << tr("The currency is invalid.");
  }

  if (m_cboType->currentData().toInt() == AccountType::INVESTMENT &&
      m_cboSecurity->currentIndex() == -1) {
  ...
  try {
    int idInstitution = m_cboInstitution->currentIndex() == -1
                            ? -1
    ...
    if (m_account) {
      ...
      if (m_cboSecurity->isEnabled())
        m_account->setIdSecurity(m_cboSecurity->currentData().toInt());
      m_account->setIdInstitution(idInstitution);
      m_account->setCode(m_txtCode->text());
      m_account->setNote(m_txtNote->toPlainText());
    } else {
      m_account = m_cboParent->currentAccount()->addChild(
          m_txtName->text(), m_cboType->currentData().toInt(),
          m_cboCurrency->isEnabled() ? m_cboCurrency->currentData().toString()
                                     : "",
          m_cboSecurity->isEnabled() ? m_cboSecurity->currentData().toInt()
                                     : Constants::NO_ID,
          m_chkPlaceholder->isChecked(), idInstitution, m_txtCode->text(),
    ...
    if (m_account->type() == AccountType::CREDITCARD &&
      ...
      m_account->properties()->set("creditlimit",
                                   m_txtCreditLimit->amount().toQVariant());
    } else {
    ...
    // Have to set the main currency at the end since may have changed the
    // secondary currencies...
    if (m_cboCurrency->isEnabled())
      m_account->setCurrency(m_cboCurrency->currentData().toString());
