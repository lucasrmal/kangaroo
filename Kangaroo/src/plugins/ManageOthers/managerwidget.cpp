...
BetterTableView::BetterTableView(QWidget* _parent) : QTableView(_parent) {}
...
void BetterTableView::selectionChanged(const QItemSelection& _selected,
...
ManagerWidget::ManagerWidget(ManageType _type, QWidget* _parent)
  ...
  m_actAdd = bottomToolbar->addAction(Core::icon("new-row"), tr("New"), this,
                                      SLOT(add()));

  if (m_type != ManageType::Prices) {
    m_actEdit = bottomToolbar->addAction(Core::icon("edit"), tr("Edit"), this,
                                         SLOT(edit()));
  }

  if (m_type == ManageType::Institutions || m_type == ManageType::Payees) {
    m_actMerge = bottomToolbar->addAction(Core::icon("merge"), tr("Merge"),
                                          this, SLOT(merge()));
  ...
  m_actDelete = bottomToolbar->addAction(Core::icon("trash"), tr("Delete"),
                                         this, SLOT(remove()));
  bottomToolbar->addWidget(sep1);
  m_actFind = bottomToolbar->addAction(Core::icon("find"), tr("Search"), this,
                                       SLOT(search()));
  ...
  if (_type == ManageType::Securities) {
    m_tableView->horizontalHeader()->setSortIndicator(SecurityColumn::NAME,
                                                      Qt::AscendingOrder);
  }
  ...
  switch (_type) {
    ...
    case ManageType::Institutions:
      ...
      m_tableModel =
          new InstitutionController(InstitutionController::NoEmpty, this);
      m_sortFilterProxyModel->setSourceModel(m_tableModel);
  ...
  if (m_type != ManageType::Prices) {
    m_tableView->horizontalHeader()->setSectionResizeMode(largeCol,
                                                          QHeaderView::Stretch);
    m_tableView->setSelectionBehavior(QAbstractItemView::SelectRows);

    m_tableView->verticalHeader()->hide();

    connect(m_tableView, SIGNAL(doubleClicked(QModelIndex)), this,
            SLOT(edit()));
    connect(m_tableView, SIGNAL(selectedRowsChanged(QList<int>)), this,
            SLOT(onSelectedRowsChanged(QList<int>)));
  } else {
    m_treeView->setSelectionBehavior(QAbstractItemView::SelectRows);
    connect(m_treeView->selectionModel(),
            SIGNAL(selectionChanged(QItemSelection, QItemSelection)), this,
            SLOT(onSelectionChangeTreeView()));
  }
}
...
void ManagerWidget::add() {
  switch (m_type) {
    case ManageType::Institutions: {
      FormEditInstitutionPayee formIns(
          FormEditInstitutionPayee::Edit_Institution, this);
      formIns.exec();
      break;
    }
    case ManageType::Payees: {
      FormEditInstitutionPayee formPayee(FormEditInstitutionPayee::Edit_Payee,
                                         this);
      formPayee.exec();
      break;
    }
    ...
    case ManageType::Securities: {
      FormEditSecurity frmSec(FormEditSecurity::SecurityClass::Security,
                              nullptr, this);
      frmSec.exec();
      break;
    }
    case ManageType::Indexes: {
      FormEditSecurity frmSec(FormEditSecurity::SecurityClass::Index, nullptr,
                              this);
      frmSec.exec();
      break;
    }
    case ManageType::Prices: {
      ...
      // Preselect the pair corresponding to the current index if it is a pair
      // or a rate.
...
void ManagerWidget::remove() {
  ...
  try {
    if (m_type == ManageType::Prices) {
        ...
        if (i->itemType == PriceItem::Exchange) {
          ...
          if (QMessageBox::question(
                  ...
                  tr("Delete this rate: <b>%1</b> to <b>%2</b> on %3?")
                          ...
                          .arg(to(pair->pair))
                          .arg(i->date.toString(
                              Core::instance()->dateFormat())) +
                      "<br><br>" + noUndo,
                  QMessageBox::Yes | QMessageBox::No,
                  QMessageBox::No) == QMessageBox::Yes) {
            PriceManager::instance()
                ->get(pair->pair->from(), pair->pair->to())
                ->remove(i->date);
          }
        }
      }
      ...
      for (int row : rows) {
        ids << m_tableView->model()->data(m_tableView->model()->index(row, 0),
                                          Qt::UserRole);
      }
      ...
      if (m_type == ManageType::Currencies) {
        for (QVariant id : ids) {
          if (!CurrencyManager::instance()->get(id.toString())->canRemove()) {
            QMessageBox::information(this, tr("Delete Currency"),
                                     tr("The currency %1 cannot be deleted: it "
                                        "is in use in at least one account.")
                                         .arg(CurrencyManager::instance()
                                                  ->get(id.toString())
                                                  ->name()));
            return;
          }
        ...
        for (QVariant id : ids) {
          if (!SecurityManager::instance()->get(id.toInt())->canRemove()) {
            QMessageBox::information(
                this, tr("Delete Security"),
                tr("The security %1 cannot be deleted: it is in use in at "
                   "least one account.")
      ...
      switch (m_type) {
        case ManageType::Currencies:
          if (ids.count() == 1) {
            msg = tr("Delete this currency: <b>%1?</b>")
                      .arg(CurrencyManager::instance()
                               ->get(ids[0].toString())
                               ->name());
          } else {
        ...
        case ManageType::Institutions:
          if (ids.count() == 1) {
            msg = tr("Delete this institution: <b>%1</b>? <br><br>This action "
                     "will result in all "
                     "accounts with this institution set to "
                     "<i>No Institution</i>.")
                      .arg(InstitutionManager::instance()
                               ->get(ids[0].toInt())
                               ->name());
          } else {
            msg = tr("Delete these %1 institutions? <br><br>This action will "
                     "result in all "
        ...
        case ManageType::Payees:
          if (ids.count() == 1) {
            msg =
                tr("Delete this payee: <b>%1</b>? <br><br>This action will "
                   "result in all "
                   "transactions with this payee set to "
                   "<i>No Payee</i>.")
                    .arg(PayeeManager::instance()->get(ids[0].toInt())->name());
          } else {
            msg = tr("Delete these %1 payees? <br><br>This action will result "
                     "in all "
        ...
        case ManageType::Securities:
          if (ids.count() == 1) {
            msg = tr("Delete this security: <b>%1</b>?")
        ...
        case ManageType::Indexes:
          if (ids.count() == 1) {
            msg = tr("Delete this index: <b>%1</b>?")
                      .arg(SecurityManager::instance()
                               ->get(ids[0].toInt())
                               ->name());
          } else {
      ...
      if (QMessageBox::question(this, title, msg + "<br><br>" + noUndo,
                                QMessageBox::Yes | QMessageBox::No,
                                QMessageBox::No) == QMessageBox::Yes) {
        switch (m_type) {
          case ManageType::Currencies:
...
void ManagerWidget::edit() {
  if (m_type != ManageType::Prices) {
    ...
    QVariant id =
        m_tableView->model()->data(m_tableView->currentIndex(), Qt::UserRole);

    switch (m_type) {
      case ManageType::Institutions: {
        FormEditInstitutionPayee formIns(
            InstitutionManager::instance()->get(id.toInt()), this);
        formIns.exec();
        break;
      }
      case ManageType::Payees: {
        FormEditInstitutionPayee formPayee(
            PayeeManager::instance()->get(id.toInt()), this);
        formPayee.exec();
        break;
      }
      case ManageType::Currencies: {
        FormEditCurrency frmCur(CurrencyManager::instance()->get(id.toString()),
                                this);
        frmCur.exec();
        break;
      }
      case ManageType::Securities: {
        FormEditSecurity frmSec(FormEditSecurity::SecurityClass::Security,
                                SecurityManager::instance()->get(id.toInt()),
                                this);
        frmSec.exec();
        break;
      }
      case ManageType::Indexes: {
        FormEditSecurity frmSec(FormEditSecurity::SecurityClass::Index,
                                SecurityManager::instance()->get(id.toInt()),
                                this);
        frmSec.exec();
        break;
...
void ManagerWidget::merge() {
  switch (m_type) {
    ...
    case ManageType::Payees: {
      ...
      for (int row : rows) {
        ids.insert(m_tableView->model()
                       ->data(m_tableView->model()->index(row, 0), Qt::UserRole)
                       .toInt());
      }
